...
async function main(): Promise<void> {
  ...
  await assertOk(
    'login should return a user id',
    Boolean(loginBody.data?.user?.id),
  );
  const userId = loginBody.data?.user?.id as string;
  const initialBranchId = loginBody.data?.session?.activeBranchId ?? null;
  await assertOk(
    'login should return permissions',
    Array.isArray(loginBody.data?.session?.permissions),
  );
  ...
  const permissionMatrixResponse = await fetch(
    `${baseUrl}/api/v1/reports/iam/permission-matrix`,
    {
      headers: {
        cookie: cookieHeader,
      },
  ...
  await assertOk(
    'permission matrix should succeed',
    permissionMatrixResponse.ok,
  );
  const permissionMatrixBody =
    (await permissionMatrixResponse.json()) as JsonResponse<{
      items?: unknown[];
      total?: number;
    }>;
  await assertOk(
    'permission matrix should return items',
    Array.isArray(permissionMatrixBody.data?.items),
  );

  const branchesResponse = await fetch(
    `${baseUrl}/api/v1/users/${userId}/branches`,
    {
      headers: {
        cookie: cookieHeader,
      },
  ...
  const branchesBody = (await branchesResponse.json()) as JsonResponse<{
    items?: BranchItem[];
  }>;
  const targetBranchId =
    branchesBody.data?.items?.find(
      (branch) => branch.branchId !== initialBranchId,
    )?.branchId ?? null;
  await assertOk('branch switch target should exist', Boolean(targetBranchId));

  const switchBranchResponse = await fetch(
    `${baseUrl}/api/v1/auth/switch-branch`,
    {
      method: 'POST',
      headers: {
  ...
  await assertOk(
    'branch switch should return auth cookies',
    switchedCookieHeader.length > 0,
  );
  const authenticatedCookieHeader = mergeCookieHeaders(
    cookieHeader,
    switchedCookieHeader,
  );
  const switchBranchBody =
    (await switchBranchResponse.json()) as JsonResponse<SessionShape>;
  await assertOk(
    'branch switch should update active branch',
    switchBranchBody.data?.session?.activeBranchId === targetBranchId,
  );

  const defaultBranchResponse = await fetch(
    `${baseUrl}/api/v1/users/${userId}/branches/default`,
    {
      method: 'PUT',
      headers: {
        'content-type': 'application/json',
  ...
  await assertOk(
    'default branch update should succeed',
    defaultBranchResponse.ok,
  );

  const auditResponse = await fetch(
    `${baseUrl}/api/v1/audit?entityType=UserBranchAccess&action=iam.user.default-branch-changed&pageSize=5`,
    {
      headers: {
        cookie: authenticatedCookieHeader,
      },
  ...
  const auditBody = (await auditResponse.json()) as JsonResponse<{
    items?: AuditItem[];
  }>;
  await assertOk(
    'audit trail should include default branch change',
    Boolean(
      auditBody.data?.items?.some(
        (item) =>
          item.action === 'iam.user.default-branch-changed' &&
          item.entityType === 'UserBranchAccess',
      ),
    ),
  );

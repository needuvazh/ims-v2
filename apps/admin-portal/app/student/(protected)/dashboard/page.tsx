...
export default function StudentDashboardPage() {
  ...
  const upcomingEvents = [
    { title: 'Process Safety Quiz (Unit 3)', time: 'Today, 10:00 AM', type: 'Quiz', color: 'text-rose-600 bg-rose-50' },
    { title: 'Risk Assessment Lab Practical', time: 'Tomorrow, 02:00 PM', type: 'Practical', color: 'text-violet-600 bg-violet-50' },
  ...
  return (
    <div className="space-y-8">
      {/* Welcome Banner */}
      <div className="relative overflow-hidden rounded-[32px] bg-gradient-to-r from-rose-950 via-slate-900 to-pink-950 p-6 md:p-8 text-white shadow-xl">
        <div className="absolute inset-0 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] opacity-20 mix-blend-overlay pointer-events-none" />
        <div className="absolute -right-16 -top-16 h-48 w-48 rounded-full bg-pink-500/20 blur-3xl pointer-events-none" />
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          <div className="space-y-2">
            <span className="inline-flex items-center gap-1.5 rounded-full bg-pink-500/20 px-3 py-1 text-xs font-bold uppercase tracking-wider text-pink-300 backdrop-blur-sm">
              <GraduationCap className="h-3.5 w-3.5" />
              Active Enrollment
            </span>
            <h1 className="text-3xl md:text-4xl font-black tracking-tight">
              Welcome back, <span className="bg-gradient-to-r from-pink-300 via-rose-300 to-violet-300 bg-clip-text text-transparent">Ahmad Al-Saud</span>
            </h1>
          ...
          <div className="flex items-center gap-3 self-start md:self-auto rounded-2xl bg-white/5 border border-white/10 p-3.5 backdrop-blur-md">
            <div className="h-10 w-10 rounded-xl bg-pink-500/20 flex items-center justify-center text-pink-300 font-bold">
              83%
            </div>
            <div>
              <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Average Attendance</p>
              <p className="text-xs font-black text-pink-300">EXCELLENT STANDING</p>
            </div>
          </div>
        </div>
      </div>
                ...
                >
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div className="space-y-1">
                      ...
                      <h3 className="text-xl font-bold text-[color:var(--ims-ink)] group-hover:text-pink-600 transition-colors">
                        {course.title}
                      </h3>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <Link href="/student/fees" className="w-full">
                <Button className="w-full py-3.5 rounded-2xl bg-gradient-to-r from-pink-500 to-rose-500 text-white font-bold shadow-lg shadow-pink-500/20 hover:shadow-pink-500/40 hover:-translate-y-0.5 transition-all flex items-center justify-center gap-2">

...
export default function ForgotPasswordPage() {
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    const errors = parseForgotFieldErrors(new FormData(event.currentTarget));
      ...
      <AnimatePresence mode="wait">
        {state.success ? (
          ...
          >
            ...
            <Link
              href="/sign-in"
              className="flex w-full items-center justify-center gap-2 rounded-2xl bg-slate-950 py-4 text-sm font-bold text-white shadow-lg transition-colors hover:bg-slate-800"
            >
              Return to Login
            </Link>
          </motion.div>

...
import { ArrowRight, Award, CalendarDays, CheckCircle2, Clock, GraduationCap, MapPin, ShieldCheck, Users } from 'lucide-react';
import {
  PublicShell,
  HeroSection,
  SectionHeading,
...
async function fetchCourse(slug: string): Promise<PublicCourseDetail | null> {
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL ?? 'http://localhost:3000'}/api/public/courses/${slug}`, {
      next: { revalidate: 300 },
    });
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return {
    ...
    description: course.descriptionEnglish ?? `Learn ${course.nameEnglish} at Al-Saud Training Institute. Hands-on training with industry-recognized certification.`,
    openGraph: {
      title: `${course.nameEnglish} | Al-Saud Training Institute`,
...
export default async function CourseDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const totalPrice = c.basePrice && c.taxPercentage
    ? (parseFloat(c.basePrice) * (1 + parseFloat(c.taxPercentage) / 100)).toFixed(3)
    : null;
        ...
        <div className="grid gap-12 lg:grid-cols-[1.2fr_0.8fr]">
          ...
          <div className="space-y-10">
            <div className="rounded-[2.5rem] border border-border-light bg-white p-8 shadow-xl shadow-primary-950/5">
              <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-accent-600">What this course covers</p>
              <h3 className="mt-4 font-display text-2xl font-bold tracking-tight text-neutral-900">Course Overview</h3>
              {c.descriptionEnglish ? (
                <p className="mt-4 leading-relaxed text-neutral-600">{c.descriptionEnglish}</p>
              ) : (
                <p className="mt-4 leading-relaxed text-neutral-600">
                  This course provides comprehensive hands-on training designed to prepare you for real-world operation.
                  Contact our admissions team for detailed course content and learning outcomes.
                </p>
              )}
              <div className="mt-6">
                <BulletList
            ...
            {c.batches.length > 0 && (
              <div className="rounded-[2.5rem] border border-border-light bg-white p-8 shadow-xl shadow-primary-950/5">
                <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-accent-600">Schedule</p>
                <h3 className="mt-4 font-display text-2xl font-bold tracking-tight text-neutral-900">Upcoming Batches</h3>
                <div className="mt-6 space-y-4">
                  {c.batches.map((batch) => (
                    <div
                      key={batch.id}
                      className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 rounded-2xl border border-border-light p-5 hover:border-border-accent transition-colors"
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="font-bold text-neutral-900">{batch.batchName}</p>
                          <p className="text-xs text-neutral-500">
                            {new Date(batch.startDate).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}
                            {' - '}
                            {new Date(batch.endDate).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}
                          </p>
                          {batch.trainerName && (
                            <p className="text-xs text-neutral-500 mt-1">Trainer: {batch.trainerName}</p>
                          )}
                        </div>
                      </div>
                      <div className="flex items-center gap-4">
                        <div className="text-right">
                          <p className="text-sm font-bold text-neutral-900">
                            {batch.availableSeats > 0 ? `${batch.availableSeats} seats left` : 'Waitlist'}
                          </p>
          ...
          <div className="space-y-6">
            <div className="rounded-[2.5rem] border border-border-light bg-white p-8 shadow-xl shadow-primary-950/5">
              ...
              <h3 className="mt-4 font-display text-2xl font-bold tracking-tight text-neutral-900">{c.nameEnglish}</h3>

              <div className="mt-6 space-y-4">
                <div className="flex items-center justify-between py-3 border-b border-border-light">
                  <span className="text-sm text-neutral-500">Duration</span>
                  <span className="text-sm font-bold text-neutral-900">{durationLabel}</span>
                </div>
                <div className="flex items-center justify-between py-3 border-b border-border-light">
                  <span className="text-sm text-neutral-500">Base Price</span>
                  <span className="text-sm font-bold text-neutral-900">{priceLabel}</span>
                </div>
                {totalPrice && totalPrice !== priceLabel && (
                  <div className="flex items-center justify-between py-3 border-b border-border-light">
                    <span className="text-sm text-neutral-500">Total (incl. tax)</span>
                    <span className="text-sm font-bold text-neutral-900">{c.currency ?? 'OMR'} {totalPrice}</span>
                  </div>
                )}
                <div className="flex items-center justify-between py-3 border-b border-border-light">
                  <span className="text-sm text-neutral-500">Category</span>
                  <span className="text-sm font-bold text-neutral-900">{c.categoryName ?? 'General'}</span>
                </div>
                <div className="flex items-center justify-between py-3">
                  <span className="text-sm text-neutral-500">Course Code</span>
                  <span className="text-sm font-bold text-neutral-900 font-mono">{c.courseCode}</span>
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="rounded-2xl bg-gradient-to-br from-primary-50 to-primary-100 p-5 ring-1 ring-border-light">
                ...
                <p className="mt-3 text-[10px] font-bold uppercase tracking-[0.2em] text-neutral-500">Training</p>
                <p className="mt-1 font-display text-lg font-bold text-neutral-900">Hands-on</p>
              </div>
              <div className="rounded-2xl bg-gradient-to-br from-accent-50 to-accent-100 p-5 ring-1 ring-border-light">
                ...
                <p className="mt-3 text-[10px] font-bold uppercase tracking-[0.2em] text-neutral-500">Safety</p>
                <p className="mt-1 font-display text-lg font-bold text-neutral-900">Certified</p>
              </div>
              <div className="rounded-2xl bg-gradient-to-br from-green-50 to-green-100 p-5 ring-1 ring-border-light">
                <Users className="h-6 w-6 text-green-700" />
                <p className="mt-3 text-[10px] font-bold uppercase tracking-[0.2em] text-neutral-500">Group</p>
                <p className="mt-1 font-display text-lg font-bold text-neutral-900">Available</p>
              </div>
              <div className="rounded-2xl bg-gradient-to-br from-blue-50 to-blue-100 p-5 ring-1 ring-border-light">
                <Award className="h-6 w-6 text-blue-700" />
                <p className="mt-3 text-[10px] font-bold uppercase tracking-[0.2em] text-neutral-500">Outcome</p>
                <p className="mt-1 font-display text-lg font-bold text-neutral-900">Certificate</p>
              </div>
            </div>
          </div>
        </div>

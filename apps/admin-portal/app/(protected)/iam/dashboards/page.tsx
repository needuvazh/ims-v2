...
export default async function IamDashboardsPage() {
  ...
  const [
    ...
    roleService.listRoles(1, 1000, {
      actorId: session.userId,
      actorPermissions: session.permissions,
      activeBranchId: session.activeBranchId,
    }),
    permissionService.searchPermissions(undefined, undefined, {
      actorId: session.userId,
      actorPermissions: session.permissions,
      activeBranchId: session.activeBranchId,
    }),
    securityPolicyService.getSecurityPolicy({
      actorId: session.userId,
      actorPermissions: session.permissions,
      activeBranchId: session.activeBranchId,
    }),
    auditQueryService.listAuditLogs({ module: 'iam' }, 1, 5, {
      actorId: session.userId,
      actorPermissions: session.permissions,
      activeBranchId: session.activeBranchId,
    }),
    loginHistoryQueryService.listSecurityLoginHistory({}, 1, 5, {
      actorId: session.userId,
      actorPermissions: session.permissions,
      activeBranchId: session.activeBranchId,
    }),
    loginHistoryQueryService
      .listSecurityLoginHistory({}, 1, 1, {
        actorId: session.userId,
        actorPermissions: session.permissions,
        activeBranchId: session.activeBranchId,
      })
      .then((r) => r.total),
    loginHistoryQueryService
      .listSecurityLoginHistory({ status: 'Failure' }, 1, 1, {
        actorId: session.userId,
        actorPermissions: session.permissions,
        activeBranchId: session.activeBranchId,
      })
  ...
  const lockedUsersCount = users.filter(
    (u: any) => u.status === 'Locked',
  ).length;
  const pendingUsersCount = users.filter(
    (u: any) => u.status === 'PendingActivation',
  ).length;
  ...
  const policyRows = [
    {
      ...
      desc: 'Lockout threshold',
    },
    {
      label: 'Lockout Duration',
      value: `${policy.lockoutDurationMinutes} min`,
      desc: 'Cool-off duration',
    },
    {
      label: 'Password Min Length',
      value: policy.passwordMinLength,
      desc: 'Minimum character rule',
    },
    {
      label: 'Password Expiry',
      value: `${policy.passwordExpiryDays} days`,
      desc: 'Password cycle time',
    },
    {
      label: 'Session Lifetime',
      value: `${policy.accessTokenExpiryMinutes} min`,
      desc: 'Token lifespan',
    },
    {
      label: 'Concurrent Limit',
      value: policy.maxConcurrentSessions,
      desc: 'Max active devices',
    },
  ];
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          <div className="space-y-2">
            ...
            <h1 className="text-3xl md:text-4xl font-black tracking-tight">
              Welcome,{' '}
              <span className="bg-gradient-to-r from-indigo-200 via-sky-100 to-white bg-clip-text text-transparent">
                {session?.displayName ?? 'Operator'}
              </span>
            </h1>
            <p className="text-sm text-neutral-300 font-medium">
              IAM and branch scope analytics for{' '}
              <span className="text-indigo-300 font-bold">{branchName}</span>.
            </p>
          </div>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
              <div>
                ...
                <CardDescription>
                  Live session records currently active in the directory.
                </CardDescription>
              </div>
                        ...
                        <TableRow key={s.id}>
                          <TableCell>
                            <div className="flex items-center gap-2">
                              ...
                              <div>
                                <p className="font-semibold text-xs text-[color:var(--ims-ink)]">
                                  {userFullName}
                                </p>
                          ...
                          <TableCell
                            className="text-xs text-[color:var(--ims-muted)] max-w-[150px] truncate"
                            title={s.userAgent ?? ''}
                          >
                            {s.userAgent
                              ? s.userAgent.split(' ')[0]
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
              <div>
                ...
                <CardDescription>
                  Most recently added administrators, coordinators, and
                  counselors.
                </CardDescription>
              </div>
              <div className="flex items-center gap-2">
                ...
                <TableBody>
                  {recentUsers.map((u: any) => (
                    <TableRow key={u.id}>
                      ...
                      <TableCell className="text-xs font-semibold text-[color:var(--ims-muted)]">
                        {u.userType}
                      </TableCell>
                      <TableCell>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
              <div>
                ...
                <CardDescription>
                  Auditing success/failure cycles of user logins.
                </CardDescription>
              </div>
                  ...
                  <TableBody>
                    {loginsResult.items.map((log) => (
                      <TableRow key={log.id}>
                        ...
                        <TableCell className="font-semibold text-xs text-[color:var(--ims-ink)]">
                          {log.attemptedEmail}
                        </TableCell>
                        <TableCell>
                          <Badge
                        ...
                        <TableCell
                          className="text-xs text-[color:var(--ims-error)] max-w-[150px] truncate"
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
              <div>
                ...
                <CardDescription>
                  Latest access policy, user status, and security audit logs.
                </CardDescription>
              </div>
              <Link href="/iam/audit">
                <Button variant="ghost" size="sm" className="h-8 text-xs">
                ...
                <Table>
                  ...
                  <TableBody>
                    {auditsResult.items.map((a) => (
                      <TableRow key={a.id}>
                        <TableCell className="text-xs text-[color:var(--ims-muted)] whitespace-nowrap">
                        ...
                        <TableCell className="text-xs text-[color:var(--ims-muted)]">
                          {a.branchId
                            ? `Branch (${a.branchId.substring(0, 8)})`
                            : 'Global'}
                        </TableCell>
                        <TableCell
                          className="text-xs text-[color:var(--ims-muted)] max-w-[150px] truncate"
        ...
        <div className="space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Active components catalog in the security domain.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="rounded-xl bg-slate-50 border border-slate-100 p-4 text-center">
                  ...
                  <p className="mt-2 text-3xl font-black text-slate-800">
                    {rolesResult.total ?? rolesResult.length}
                  </p>
                </div>
                <div className="rounded-xl bg-slate-50 border border-slate-100 p-4 text-center">
                  <p className="text-[10px] font-bold uppercase tracking-widest text-[color:var(--ims-muted)]">
              ...
              <div className="space-y-3">
                <p className="text-xs font-bold text-[color:var(--ims-muted)] uppercase tracking-wider">
                  Access Directory Shortcuts
                </p>
                <div className="flex flex-col gap-2">
                  <Link
                    href="/iam/roles"
                    className="flex items-center justify-between rounded-lg border border-slate-100 p-2 text-xs font-semibold hover:bg-slate-50 transition-colors"
                  >
                    <span className="flex items-center gap-2">
                      <UserCheck className="h-4 w-4 text-indigo-500" /> Manage
                      Roles
                    </span>
                  ...
                  >
                    <span className="flex items-center gap-2">
                      <Key className="h-4 w-4 text-indigo-500" /> Catalog
                      Permissions
                    </span>
                    <ArrowRight className="h-3.5 w-3.5 text-[color:var(--ims-muted)]" />
                  </Link>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
              <div>
                ...
                <CardDescription>
                  Hardened system rule limits and timeout schedules.
                </CardDescription>
              </div>
                ...
                {policyRows.map((row) => (
                  ...
                  >
                    <div>
                      ...
                      <p className="text-[9px] text-[color:var(--ims-muted)]">
                        {row.desc}
                      </p>
                    </div>
                    <Badge
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Jump straight to filtered tables and export schedules.
              </CardDescription>
            </CardHeader>
            <CardContent className="flex flex-col gap-2">
              <Link
                href="/iam/reports"
                className="flex items-center justify-between rounded-lg bg-indigo-50/50 hover:bg-indigo-50 border border-indigo-100/50 p-3 text-xs font-semibold transition-colors group"
              >
                <span className="flex items-center gap-2 text-indigo-900">
                  <Activity className="h-4 w-4 text-indigo-600" /> Advanced IAM
                  Reports
                </span>
              ...
              <Link
                ...
                className="flex items-center justify-between rounded-lg bg-slate-50 hover:bg-slate-100 border border-slate-100 p-3 text-xs font-semibold transition-colors group"
              >
                <span className="flex items-center gap-2 text-slate-900">
                  <Clock className="h-4 w-4 text-slate-600" /> Export Downloads
                  Queue
                </span>

  ...
  return withServerActionObservability(
    async () => {
      ...
      try {
        ...
        const effectiveStartDateStr = formData.get('effectiveStartDate')
          ? String(formData.get('effectiveStartDate'))
          : undefined;
        const effectiveEndDateStr = formData.get('effectiveEndDate')
        ...
        await userService.createUser(
          {
            ...
            effectiveStartDate: effectiveStartDateStr
              ? new Date(effectiveStartDateStr)
              : undefined,
            effectiveEndDate: effectiveEndDateStr
              ? new Date(effectiveEndDateStr)
              : null,
          },
          { actorId },
        );
    ...
    async () => {
      ...
      try {
        ...
        const branchIds = formData
          ...
          .map((value) => String(value))
          .filter((value) => value.trim() !== '');
        const userStatusRaw = formData.get('status')
        ...
        await userService.updateUser(
          userId,
          {
            ...
            userType: formData.get('userType')
              ? (String(formData.get('userType')) as UserType)
              : undefined,
            status: isUserStatus(userStatusRaw ?? '')
              ? userStatusRaw
              : undefined,
        ...
        logger.info('iam.user.update.succeeded', {
          status: 'success',
          entityId: userId,
        });
        revalidatePath('/iam');
...
export async function userLifecycleAction(
  userId: string,
  action:
    | 'activate'
    | 'suspend'
...
): Promise<ActionResult<{ activationLink?: string; resetLink?: string }>> {
  return withServerActionObservability(
    async () => {
      const logger = createStructuredLogger(getCurrentRequestContext() ?? {});
      ...
      try {
        ...
        switch (action) {
          ...
          case 'adminResetPassword':
            resetLink = await userService.adminResetPassword(userId, {
              actorId,
            });
            break;
          case 'resendActivationEmail':
            activationLink = await userService.resendActivationEmail(userId, {
              actorId,
            });
            break;
        }

        logger.info(`iam.user.lifecycle.${action}.succeeded`, {
          status: 'success',
          entityId: userId,
        });
        revalidatePath('/iam');
        ...
        logger.error(`iam.user.lifecycle.${action}.failed`, {
          status: 'failed',

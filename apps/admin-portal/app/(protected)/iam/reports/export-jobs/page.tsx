...
export default async function IamExportJobsPage({
  searchParams,
}: {
  ...
  const recentJobs = await exportJobRepository.listByUser(
    session.userId as never,
  );
  const selectedJob = jobId
    ? await exportJobRepository.findById(jobId as never)
    : null;
    ...
    <div className="space-y-8">
        ...
        breadcrumbs={
          <Breadcrumbs
            items={[
              ...
              {
                label: 'Export Jobs',
                icon: <FileClock className="h-3.5 w-3.5 text-slate-500" />,
              },
            ]}
          />
        }
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <FileClock className="h-5 w-5" /> Look up a job
          </CardTitle>
          <CardDescription>
            Paste an export job id to inspect the current status and download
            link.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form
            ...
            action="/iam/reports/export-jobs"
            method="get"
          >
            <Input
              name="jobId"
      ...
      {selectedJob ? (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {selectedJob.reportType} export created on{' '}
          ...
          <CardContent className="space-y-3 text-sm">
            ...
            {selectedJob.fileUrl ? (
              <Link
                href={`/api/v1/reports/iam/export-jobs/${selectedJob.id}/download`}
                className="inline-flex items-center gap-2 font-semibold text-[color:var(--ims-brass)] hover:underline"
              >
                <Download className="h-4 w-4" /> Download export
              </Link>
            ) : (
              <p className="text-[color:var(--ims-muted)]">
                The download link appears when processing completes.
              </p>
            )}
          </CardContent>
        </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {recentJobs.length} export job(s) requested by the current user.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
              ...
              {recentJobs.map((job) => (
                <TableRow key={job.id}>
                  ...
                  <TableCell>
                    <Link
                      href={{
                        pathname: '/iam/reports/export-jobs',
                        query: { jobId: job.id },
                      }}

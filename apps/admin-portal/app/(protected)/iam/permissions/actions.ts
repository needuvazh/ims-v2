  ...
  return withServerActionObservability(
    async () => {
      ...
      try {
        ...
        await permissionService.updatePermission(permissionId as Uuid, parsed, {
          actorId,
        });

        logger.info('iam.permission.update.succeeded', {
          status: 'success',
...
export async function archivePermissionAction(
  permissionId: string,
): Promise<ActionResult> {
  return withServerActionObservability(
    async () => {
      ...
      try {
        ...
        await permissionService.archivePermission(permissionId as Uuid, {
          actorId,
        });

        logger.info('iam.permission.archive.succeeded', {
          status: 'success',
          entityId: permissionId,
        });
        revalidatePath('/iam/permissions');
        ...
        logger.error('iam.permission.archive.failed', {
          status: 'failed',
          message: 'Failed to archive permission.',
          error: err as Error,
        });
        return {
          success: false,

...
export function EnrollmentDetailsClient({
    ...
    startTransition(async () => {
      try {
        const response = await fetch(
          `/api/v1/enrollments/${enrollment.id}/${action}`,
          {
            method: 'POST',
            headers: {
        ...
        if (!response.ok) {
          throw new Error(
            data.messageEnglish || `Failed to ${action} enrollment.`,
          );
        }
  ...
  const handleDropSubmit = async (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      try {
        const response = await fetch(
          `/api/v1/enrollments/${enrollment.id}/drop`,
          {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
  ...
  const handlePaymentSubmit = async (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      try {
        const response = await fetch(
          `/api/v1/enrollments/${enrollment.id}/walk-in-payment`,
          {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
        ...
        if (!response.ok) {
          throw new Error(
            data.messageEnglish || 'Failed to record walk-in payment.',
          );
        }

        toast.success(
          'Walk-in payment recorded successfully. Enrollment Confirmed.',
        );
  ...
  const canPay =
    ...
    sessionPermissions.includes('enrollment.walk-in-payment');
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between border-b border-slate-100 pb-5">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <Badge
              variant={getStatusBadgeVariant(enrollment.enrollmentStatus)}
              className="ml-2"
            >
          ...
          <p className="text-sm text-slate-500 pl-7">
            Configure learning batch seats, resolve fees, and execute status
            updates.
          </p>
        </div>
      </div>
          ...
          <Card className="bg-white/80 border border-[color:var(--ims-border)] shadow-sm rounded-2xl p-5 space-y-4">
            ...
            {history.length === 0 ? (
              <p className="text-sm text-slate-400 italic">
                No lifecycle transition events recorded.
              </p>
            ) : (
              ...
              {!canSubmit &&
                ...
                !canPay && (
                  <p className="text-xs text-slate-400 italic text-center py-2">
                    No workflow actions are currently available for this
                    enrollment status.
                  </p>
                )}
            </div>
        ...
        <DialogContent>
          <form onSubmit={handleDropSubmit} className="space-y-4">
            <DialogHeader>
              ...
              <DialogDescription>
                This action is terminal and will withdraw the student from the
                batch, releasing their seat capacity.
              </DialogDescription>
            </DialogHeader>
        ...
        <DialogContent>
          <form onSubmit={handlePaymentSubmit} className="space-y-4">
            <DialogHeader>
              ...
              <DialogDescription>
                Record the physical payment collected at the branch counter to
                confirm this walk-in enrollment.
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="space-y-4 py-2 text-sm">
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-700 font-mono">
                  Total Pending Amount: OMR{' '}
                  {Number(enrollment.finalAmount).toFixed(3)}
                </label>

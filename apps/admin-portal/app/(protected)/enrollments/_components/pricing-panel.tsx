    ...
    <Card className="bg-white/80 backdrop-blur-md border border-[color:var(--ims-border)] shadow-sm rounded-2xl p-5 space-y-4">
      <div className="flex items-center justify-between border-b border-slate-100 pb-2">
        <div className="flex items-center gap-2">
          ...
          <h4 className="font-semibold text-slate-800 text-sm uppercase">Pricing Resolution Details</h4>
        </div>
      ...
      <div className="space-y-2.5 text-sm">
        <div className="flex justify-between">
          ...
          <span className="font-medium text-slate-700 font-mono">OMR {Number(resolvedPrice).toFixed(3)}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-slate-500">Applied Discount:</span>
          <span className="font-semibold text-emerald-600 font-mono">- OMR {Number(resolvedDiscount).toFixed(3)}</span>
        </div>
      ...
      {priceEvaluationTimestamp && (
        <div className="pt-2 border-t border-slate-100 flex items-center gap-1.5 text-[10px] text-slate-400 font-medium">
          ...
          <span>Evaluated on {new Date(priceEvaluationTimestamp).toLocaleString()}</span>
        </div>
      )}
    </Card>
  );

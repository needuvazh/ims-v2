...
function buildAttendanceActionFailure(error: any) {
  ...
  if (message.includes('ERR_ATT_SESSION_NOT_FOUND')) {
    return { success: false as const, error: 'Attendance session or source delivery session was not found.' };
  }
  if (message.includes('ERR_ATT_SESSION_BRANCH_FORBIDDEN')) {
    return { success: false as const, error: 'You are not authorized to open attendance for this branch.' };
  }
  if (message.includes('ERR_ATT_SESSION_LOCKED')) {
    return { success: false as const, error: 'This attendance session cannot be edited in its current state.' };
  }
...
export async function openAttendanceSessionAction(sessionId: string) {
  try {
    ...
    const { attendanceService, branchScopeResolver } = await import('../../lib/runtime');
    ...
    if (!sourceSession || sourceSession.isDeleted || sourceSession.batch.isDeleted) {
      throw new Error('ERR_ATT_SESSION_NOT_FOUND');
    }

    if (allowedBranchIds.length > 0 && !allowedBranchIds.includes(sourceSession.batch.branchId)) {
    ...
    const attendanceSession = await prisma.$transaction(async (tx) => {
      ...
      if (existing) {
        if (existing.records.length === 0) {
          await attendanceService.generateRoster(existing.id, attendanceContext, tx);
        }
        return existing;
      }

      const opened = await attendanceService.openSession({ sessionId, notes: null }, attendanceContext, tx);

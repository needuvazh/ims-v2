...
import { ClipboardList, Loader2, MessageSquarePlus, Save, Send, Sparkles } from 'lucide-react';
import {
  Alert,
  Badge,
  Button,
...
type AttendanceSessionStatus = 'Draft' | 'Open' | 'Submitted' | 'Locked' | 'Reopened' | 'Cancelled';
type AttendanceRecordStatus = 'Present' | 'Absent' | 'Late' | 'Excused' | 'Unmarked';
...
type AttendanceRosterRecord = {
  ...
  correctionStatus: 'Pending' | 'Approved' | 'Rejected' | 'Cancelled' | 'None' | null;
  enrollment: {
    id: string;
    studentProfile: {
      id: string;
...
function sessionBadge(status: AttendanceSessionStatus) {
  if (status === 'Open' || status === 'Draft') return <Badge variant="info">{status}</Badge>;
...
async function postAttendanceAction(path: string, body?: unknown) {
  ...
  if (!response.ok || !payload?.success) {
    throw new Error(payload?.messageEnglish || payload?.error || 'Unable to complete attendance action.');
  }

  return payload.data;
}

const attendanceStatuses: AttendanceRecordStatus[] = ['Present', 'Absent', 'Late', 'Excused', 'Unmarked'];
...
export function AttendanceRosterEditor({
  ...
  const [correctionTargetStatus, setCorrectionTargetStatus] = useState<AttendanceRecordStatus>('Present');
  ...
  const updateRow = (attendanceRecordId: string, patch: Partial<RowState>) => {
    setRows((current) =>
      current.map((row) =>
        row.attendanceRecordId === attendanceRecordId ? { ...row, ...patch } : row,
      ),
    );
  };
  ...
  const validateRows = (forSubmit: boolean) => {
    ...
    for (const row of rows) {
      ...
      if (row.status === 'Late' && (!row.lateMinutes.trim() || Number(row.lateMinutes) <= 0)) {
  ...
  const handleGenerateRoster = () => {
    ...
    if (!canGenerateRoster) {
      setError('You do not have permission to generate the roster for this session.');
      return;
    }

    startTransition(async () => {
      try {
        await postAttendanceAction(`/api/v1/attendance/sessions/${sessionId}/roster`);
        toast.success('Attendance roster generated.');
        router.refresh();
      } catch (submissionError) {
        toast.error(submissionError instanceof Error ? submissionError.message : 'Failed to generate roster.');
      }
    });
  };
  ...
  const handleSaveDraft = () => {
    setError(null);
    if (!editable) {
      setError('This attendance session is locked or cannot be edited in its current state.');
      return;
    }
    ...
    startTransition(async () => {
      try {
        await postAttendanceAction(`/api/v1/attendance/sessions/${sessionId}/mark`, toPayload(rows));
        toast.success('Attendance draft saved.');
        router.refresh();
      } catch (submissionError) {
        toast.error(submissionError instanceof Error ? submissionError.message : 'Failed to save attendance draft.');
      }
    });
  };
  ...
  const handleSubmit = () => {
    setError(null);
    if (!editable) {
      setError('This attendance session is locked or cannot be submitted in its current state.');
      return;
    }
    ...
    startTransition(async () => {
      try {
        await postAttendanceAction(`/api/v1/attendance/sessions/${sessionId}/mark`, toPayload(rows));
        await postAttendanceAction(`/api/v1/attendance/sessions/${sessionId}/submit`, {
          allowUnmarked: false,
          reason: null,
        });
        ...
        toast.error(submissionError instanceof Error ? submissionError.message : 'Failed to submit attendance.');
      }
    });
  };
  ...
  const openCorrectionDialog = (row: RowState) => {
    ...
    if (row.correctionStatus === 'Pending') {
      setError('A correction is already pending for this record. Wait for approval or rejection before requesting another one.');
      return;
    }
    setCorrectionDraft({
      attendanceRecordId: row.attendanceRecordId,
      currentStatus: row.status,
      newStatus: attendanceStatuses.find((status) => status !== row.status) ?? 'Present',
      reason: '',
      studentName: row.studentName,
    });
    setCorrectionReason('');
    setCorrectionTargetStatus(attendanceStatuses.find((status) => status !== row.status) ?? 'Present');
  };
  ...
  const handleRequestCorrection = () => {
    ...
    startTransition(async () => {
      try {
        ...
        toast.error(submissionError instanceof Error ? submissionError.message : 'Failed to request correction.');
      }
    });
  };
  ...
  return (
    <div className="space-y-6">
      {error && (
        <Alert variant="error" title="Attendance action failed" description={error} />
      )}
        ...
        <CardHeader className="border-b border-[color:var(--ims-border)] bg-[color:var(--ims-surface-hover)]">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            <div className="space-y-2">
              ...
              <CardDescription>
                Use the roster below to record Present, Absent, Late, or Excused for each enrolled student.
              </CardDescription>
            </div>
        ...
        <CardContent className="space-y-6 p-4 sm:p-5 lg:p-6">
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            <div className="rounded-2xl border border-[color:var(--ims-border)] bg-[color:var(--ims-surface)] p-4">
              <p className="text-xs uppercase tracking-[0.2em] text-[color:var(--ims-muted)]">Session</p>
              <p className="mt-2 text-lg font-semibold text-[color:var(--ims-ink)]">{sessionTitleEnglish}</p>
              {sessionTitleArabic ? <p className="mt-1 text-sm text-[color:var(--ims-muted)] font-arabic">{sessionTitleArabic}</p> : null}
              <p className="mt-2 text-xs text-[color:var(--ims-muted)]">#{sessionNumber ?? '—'} | {batchCode}</p>
            </div>
            <div className="rounded-2xl border border-[color:var(--ims-border)] bg-[color:var(--ims-surface)] p-4">
              <p className="text-xs uppercase tracking-[0.2em] text-[color:var(--ims-muted)]">Branch</p>
              <p className="mt-2 text-lg font-semibold text-[color:var(--ims-ink)]">{branchName}</p>
              <p className="mt-2 text-xs text-[color:var(--ims-muted)]">Attendance date: {formatDateTime(attendanceDate)}</p>
            </div>
            <div className="rounded-2xl border border-[color:var(--ims-border)] bg-[color:var(--ims-surface)] p-4">
              <p className="text-xs uppercase tracking-[0.2em] text-[color:var(--ims-muted)]">Marked</p>
              <p className="mt-2 text-3xl font-semibold text-[color:var(--ims-ink)]">{stats.marked}</p>
              <p className="mt-2 text-xs text-[color:var(--ims-muted)]">of {stats.total} roster rows</p>
            </div>
            <div className="rounded-2xl border border-[color:var(--ims-border)] bg-[color:var(--ims-surface)] p-4">
              <p className="text-xs uppercase tracking-[0.2em] text-[color:var(--ims-muted)]">Unmarked</p>
              <p className="mt-2 text-3xl font-semibold text-[color:var(--ims-ink)]">{stats.unmarked}</p>
              <p className="mt-2 text-xs text-[color:var(--ims-muted)]">Resolve before submit</p>
            </div>
          </div>
            ...
            <div className="overflow-x-auto">
              <Table>
                <TableHeader>
                  ...
                  {canRequestCorrection ? <TableHead className="text-right">Actions</TableHead> : null}
                </TableRow>
              </TableHeader>
              <TableBody>
                {rows.map((row) => (
                    <TableRow key={row.attendanceRecordId}>
                      <TableCell>
                        <div className="space-y-0.5">
                          <div className="font-semibold text-[color:var(--ims-ink)]">{row.studentName}</div>
                          <div className="text-xs text-[color:var(--ims-muted)]">{row.studentNumber}</div>
                        </div>
                      </TableCell>
                      <TableCell className="text-sm text-[color:var(--ims-muted)]">{row.enrollmentId}</TableCell>
                      ...
                      <TableCell>
                        <Textarea
                          ...
                          rows={2}
                          placeholder={row.status === 'Excused' ? 'Reason required for excused attendance' : 'Optional internal note'}
                        />
                      </TableCell>
          ...
          <p className="text-xs text-[color:var(--ims-muted)]">
            Late attendance requires positive late minutes. Excused attendance requires a reason. Unmarked rows must be resolved before submission.
          </p>
        </CardContent>
      </Card>

      <Dialog open={Boolean(correctionDraft)} onOpenChange={(open) => !open && setCorrectionDraft(null)}>
        <DialogContent className="max-w-xl">
          <DialogHeader>
            <DialogTitle>Request Attendance Correction</DialogTitle>
            <DialogDescription>
              Create a pending correction for {correctionDraft?.studentName}. The request will go to the correction queue for approval.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4">
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="rounded-2xl border border-[color:var(--ims-border)] bg-[color:var(--ims-surface)] p-4">
                <p className="text-xs uppercase tracking-[0.2em] text-[color:var(--ims-muted)]">Current Status</p>
                <p className="mt-2 text-lg font-semibold text-[color:var(--ims-ink)]">{correctionDraft?.currentStatus ?? '—'}</p>
              </div>
              <div className="rounded-2xl border border-[color:var(--ims-border)] bg-[color:var(--ims-surface)] p-4">
                <p className="text-xs uppercase tracking-[0.2em] text-[color:var(--ims-muted)]">Target Status</p>
                <div className="mt-2">
                  <Select
                    value={correctionTargetStatus}
                    onChange={(event) => setCorrectionTargetStatus(event.target.value as AttendanceRecordStatus)}
                    options={attendanceStatuses
                      .filter((status) => status !== correctionDraft?.currentStatus)
          ...
          <DialogFooter>
            <Button type="button" variant="outline" onClick={() => setCorrectionDraft(null)}>
              Cancel
            </Button>
            <Button type="button" variant="primary" onClick={handleRequestCorrection} disabled={isPending}>

    ...
    async () => {
      ...
      try {
        ...
        const userStatusRaw = String(
          formData.get('status') ?? 'PendingActivation',
        );
        await userService.createUser(
          {
            ...
            status: isUserStatus(userStatusRaw)
              ? userStatusRaw
              : 'PendingActivation',
            roleIds: [],
            branchIds,
  ...
  return withServerActionObservability(
    async () => {
      ...
      try {
        ...
        logger.info('identity.user.update.succeeded', {
          status: 'success',
          entityId: userId,
        });
        revalidatePath('/iam');
  ...
  return withServerActionObservability(
    async () => {
      ...
      try {
        ...
        logger.info('identity.user.status.updated', {
          status: 'success',
          entityId: userId,
        });
        revalidatePath('/iam');
        ...
        if (err instanceof DomainError) {
          logger.warn('identity.user.status.update.failed', {
            status: 'failed',
            message: err.message,
            error: err,
          });
        ...
        logger.error('identity.user.status.update.failed', {
          status: 'failed',
  ...
  return withServerActionObservability(
    async () => {
      ...
      try {
        ...
        logger.info('identity.user.role.assigned', {
          status: 'success',
          entityId: userId,
          code: roleId,
        });
        ...
        if (err instanceof DomainError) {
          logger.warn('identity.user.role.assign.failed', {
            status: 'failed',
            message: err.message,
            error: err,
          });
        ...
        logger.error('identity.user.role.assign.failed', {
          status: 'failed',
  ...
  return withServerActionObservability(
    async () => {
      ...
      try {
        ...
        logger.info('identity.user.role.toggled', {
          status: 'success',
          entityId: userId,
          code: roleId,
        });
        ...
        if (err instanceof DomainError) {
          logger.warn('identity.user.role.toggle.failed', {
            status: 'failed',
            message: err.message,
            error: err,
          });
        ...
        logger.error('identity.user.role.toggle.failed', {
          status: 'failed',
...
export async function getUserRolesAction(
  userId: string,
): Promise<ActionResult<{ id: string; roleCode: string; roleName: string }[]>> {
  try {
  ...
  return withServerActionObservability(
    async () => {
      ...
      try {
        ...
        logger.info('identity.role.update.succeeded', {
          status: 'success',
          entityId: roleId,
        });
        revalidatePath('/iam');
  ...
  return withServerActionObservability(
    async () => {
      ...
      try {
        ...
        logger.info('identity.role.status.updated', {
          status: 'success',
          entityId: roleId,
        });
        revalidatePath('/iam');
        ...
        if (err instanceof DomainError) {
          logger.warn('identity.role.status.update.failed', {
            status: 'failed',
            message: err.message,
            error: err,
          });
        ...
        logger.error('identity.role.status.update.failed', {
          status: 'failed',
  ...
  return withServerActionObservability(
    async () => {
      ...
      try {
        ...
        logger.info('identity.role.permission.toggled', {
          status: 'success',
          entityId: roleId,
          code: permissionId,
        });
        ...
        if (err instanceof DomainError) {
          logger.warn('identity.role.permission.toggle.failed', {
            status: 'failed',
            message: err.message,
            error: err,
          });
        ...
        logger.error('identity.role.permission.toggle.failed', {
          status: 'failed',

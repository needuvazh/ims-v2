...
export default async function IdentityPermissionsPage(props: {
  ...
  if (moduleFilter) {
    filteredPermissions = filteredPermissions.filter(
      (p) => p.moduleCode === moduleFilter,
    );
  }

  if (typeFilter) {
    filteredPermissions = filteredPermissions.filter(
      (p) => p.permissionType === typeFilter,
    );
  }

  const uniqueModules = Array.from(
    new Set(data.permissions.map((p) => p.moduleCode)),
  ).sort();
  const uniqueTypes = Array.from(
    new Set(data.permissions.map((p) => p.permissionType)),
  ).sort();
        ...
        breadcrumbs={
          <Breadcrumbs
            items={[
              ...
              {
                label: 'Permissions',
                icon: <Key className="h-3.5 w-3.5 text-slate-500" />,
              },
            ]}
          />
        }

...
function buildDays(calendar?: BusinessCalendar) {
  const byDay = new Map(
    calendar?.operatingDays.map((day) => [day.dayOfWeek, day]),
  );
...
export function BranchOverrideForm({
  ...
  >('Draft');
  const [days, setDays] = useState<OverrideDayState[]>(() =>
    buildDays(calendar),
  );
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch(
        `/api/v1/scheduling/calendars/${calendar.id}/branch-overrides`,
        {
          method: 'POST',
      ...
      if (!response.ok) {
        throw new Error(
          json?.detail || json?.messageEnglish || 'Unable to save override.',
        );
      }
  ...
  return (
    <div className="space-y-4">
          ...
          <CardContent className="space-y-6 p-card-p">
            <div className="grid gap-4">
              ...
              <Input
                ...
                placeholder="e.g. Ramadan Hours 2026"
              />
              <div className="grid grid-cols-2 gap-4">
                <Input
                  label="Start"
            ...
            <div className="space-y-3">
              ...
              <div className="grid gap-2">
                {days.map((day, index) => (
                  <div
                    key={day.dayOfWeek}
                    className={`flex flex-col gap-3 p-3 rounded-xl border transition-all ${day.enabled ? 'border-[color:var(--ims-brass)] bg-[color:var(--ims-accent-soft)]' : 'border-[color:var(--ims-border)] bg-[color:var(--ims-background)] opacity-60'}`}
                  >
      ...
      <p className="text-[10px] text-center text-[color:var(--ims-muted)] px-4">
        Unselected days will continue to inherit institute baseline rules for{' '}
        {selectedBranchLabel}.
      </p>
    </div>
  );

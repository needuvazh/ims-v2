...
export default async function DashboardPage() {
  ...
  const mockActivities = [
    ...
    { type: 'dept', message: 'Department "Safety Engineering" created under Riyadh Branch', time: '3 hours ago', icon: BookOpen, color: 'text-violet-500 bg-violet-500/10' },
    { type: 'audit', message: 'Audit log appended for action: organization.institute_created', time: '5 hours ago', icon: Activity, color: 'text-cyan-500 bg-cyan-500/10' },
  ];

  const quickLinks = [
    { title: 'Register Institute', desc: 'Add new corporate entity', href: '/organization', icon: Building2, color: 'from-violet-500 to-fuchsia-500' },
    { title: 'Manage Branches', desc: 'Configure regional campuses', href: '/organization', icon: BookOpen, color: 'from-pink-500 to-rose-500' },
    { title: 'Configure Departments', desc: 'Organize training divisions', href: '/organization', icon: GraduationCap, color: 'from-cyan-500 to-blue-500' },
  ...
  return (
    <div className="space-y-8">
      {/* Page Header */}
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-violet-900/90 via-fuchsia-900/80 to-cyan-900/90 p-6 md:p-8 text-white shadow-xl backdrop-blur-md border border-white/20">
        ...
        <div className="absolute -right-16 -top-16 h-48 w-48 rounded-full bg-fuchsia-500/40 blur-3xl pointer-events-none" />
        <div className="absolute -left-16 -bottom-16 h-48 w-48 rounded-full bg-cyan-500/40 blur-3xl pointer-events-none" />
        
        <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          <div className="space-y-2">
            <span className="inline-flex items-center gap-1.5 rounded-full bg-fuchsia-500/30 px-3 py-1 text-xs font-bold uppercase tracking-wider text-fuchsia-100 backdrop-blur-md">
              <span className="h-1.5 w-1.5 rounded-full bg-fuchsia-300 animate-pulse" />
              Central Control Unit
            </span>
            <h1 className="text-3xl md:text-4xl font-black tracking-tight">
              Welcome back, <span className="bg-gradient-to-r from-violet-200 via-fuchsia-200 to-cyan-200 bg-clip-text text-transparent">{session?.displayName ?? 'Admin'}</span>
            </h1>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
                    ...
                    <Link key={link.title} href={link.href} className="group relative block rounded-xl border border-white/60 bg-white/60 backdrop-blur-xl p-5 transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="space-y-1">
                          <p className="font-bold text-[color:var(--ims-ink)] flex items-center gap-1 group-hover:text-violet-600 transition-colors">
                            {link.title}
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4" data-testid="institute-list">
                {summary.institutes.map((inst) => (
                  ...
                  >
                    <div className="absolute top-0 right-0 w-20 h-20 bg-gradient-to-br from-violet-500/10 to-transparent rounded-bl-full -mr-4 -mt-4 transition-transform group-hover:scale-125" />
                    
                    <div className="flex items-start justify-between gap-3 relative z-10">
                      <div className="space-y-1">
                        <p className="font-black text-[color:var(--ims-ink)] text-base group-hover:text-violet-600 transition-colors">
                          {inst.instituteName}
                        </p>
        ...
        <div className="space-y-6">
            ...
            <CardContent className="space-y-4">
              <div className="rounded-xl bg-slate-50 border border-slate-200/60 p-4 space-y-3">
                <div className="flex items-center justify-between text-xs">
                  <span className="text-slate-500 font-bold">Outbox Daemon</span>
                  <Badge variant="success">Online</Badge>
                </div>
                <div className="flex items-center justify-between text-xs">
                  <span className="text-slate-500 font-bold">Events Processed</span>
                  <span className="font-bold text-slate-800">1,248</span>
                </div>
                <div className="flex items-center justify-between text-xs">
                  <span className="text-slate-500 font-bold">Retries / Failures</span>
                  <span className="font-bold text-slate-800">0</span>
                </div>
                <div className="flex items-center justify-between text-xs">
                  <span className="text-slate-500 font-bold">Thread Worker PID</span>
                  <span className="font-mono bg-slate-200 text-slate-700 px-1.5 py-0.5 rounded text-[10px]">8472</span>
                </div>
              </div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="relative pl-4 border-l-2 border-indigo-100 space-y-5">

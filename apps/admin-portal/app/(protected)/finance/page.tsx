...
export const metadata = {
  title: 'Finance Dashboard - Admin Portal | ASTI IMS',
};
...
export default async function FinanceDashboardPage() {
  ...
  const totalInvoiced = nonDraftInvoices.reduce(
    (sum, inv) => sum + Number(inv.totalAmount),
    0,
  );
  const revenueCollected = nonDraftInvoices.reduce(
  ...
  const installmentsDueThisMonth = rawInstallments.reduce(
  ...
  const thisMonthInvoiced = nonDraftInvoices
    .filter(
      (inv) =>
        inv.invoiceDate >= startOfThisMonth &&
        inv.invoiceDate <= endOfThisMonth,
    )
    .reduce((sum, inv) => sum + Number(inv.totalAmount), 0);
  ...
  const lastMonthInvoiced = nonDraftInvoices
    .filter(
      (inv) =>
        inv.invoiceDate >= startOfLastMonth &&
        inv.invoiceDate <= endOfLastMonth,
    )
    .reduce((sum, inv) => sum + Number(inv.totalAmount), 0);
  ...
  const monthOverMonthData = Array.from({ length: maxDays }).map((_, i) => {
    ...
    const currentMonthVal = nonDraftInvoices
      .filter((inv) => {
        const d = new Date(inv.invoiceDate);
        return (
          d.getFullYear() === currentMonthYear &&
  ...
  const trendData = last6Months.map((m) => {
    const rev = nonDraftInvoices
      .filter(
        (inv) =>
          inv.invoiceDate >= m.monthStart && inv.invoiceDate <= m.monthEnd,
      )
      .reduce((sum, inv) => sum + Number(inv.totalAmount), 0);
    return {
      month: m.label,
  ...
  const paidCount = nonDraftInvoices.filter(
    (inv) => inv.status === 'Paid',
  ).length;
  const partialCount = nonDraftInvoices.filter(
    (inv) => inv.status === 'PartiallyPaid',
  ).length;
  const pendingCount = nonDraftInvoices.filter((inv) =>
    ...
    },
    {
      ...
      render: (inv: any) => {
        if (inv.studentProfile) {
          ...
          return (
            <span className="font-semibold text-slate-800">
              {p.firstName} {p.lastName} (Student)
            </span>
          );
        }
        if (inv.corporateAccount) {
          return (
            <span className="font-semibold text-slate-800">
              {inv.corporateAccount.accountName} (Corporate)
            </span>
          );
        }
        ...
        actions={
          <div className="flex gap-3">
            <Link href="/finance/invoices">
              <Button className="h-10 gap-2">Manage Invoices</Button>
            </Link>
          </div>
        }
      />
        ...
        <Card>
          ...
          <CardContent className="p-card-p space-y-3">
            <Link href="/finance/invoices/create" className="block w-full">
              ...
              >
                <Plus className="h-4 w-4 text-indigo-500" /> Create
                Student/Corporate Invoice
              </Button>
            </Link>
            <Link href="/finance/payments" className="block w-full">
              <Button
                ...
                className="w-full justify-start h-11 gap-3"
              >
                <Plus className="h-4 w-4 text-emerald-500" /> Record Client
                Payment
              </Button>
            </Link>
            <Link href="/finance/refunds" className="block w-full">
              <Button
                ...
                className="w-full justify-start h-11 gap-3"
              >
                <ArrowDownRight className="h-4 w-4 text-rose-500" /> Request
                Refund Reversal
              </Button>
            </Link>
          </CardContent>

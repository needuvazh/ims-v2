...
import { updateCorporateAccountAction } from "../actions";
import { UpdateCorporateAccountInput, UpdateCorporateAccountSchema } from "../schemas";

interface EditAccountClientFormProps {
  account: any;
  branches: Array<{ id: string; name: string }>;
  actorId: string;
}

export function EditAccountClientForm({
  account,
  branches,
  actorId,
}: EditAccountClientFormProps) {
  ...
  const {
    ...
    control,
    formState: { errors },
  } = useForm<UpdateCorporateAccountInput>({
    resolver: zodResolver(UpdateCorporateAccountSchema),
    defaultValues: {
  ...
  async function onSubmit(data: UpdateCorporateAccountInput) {
    setLoading(true);
    setServerError(null);

    try {
      await updateCorporateAccountAction(account.id, data, account.version, actorId);
      router.push(`/corporate-training/accounts/${account.id}`);
      router.refresh();
    } catch (err: any) {
      setServerError(err.message || "Failed to update corporate account");
      setLoading(false);
    }
  }
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-2 max-w-4xl mx-auto">
        ...
        <div className="space-y-5 rounded-2xl border border-[color:var(--ims-border)] bg-white/80 p-4 shadow-sm backdrop-blur-sm sm:p-5 lg:p-6">
          ...
          <FormField>
            <FormLabel>Account Code (Immutable)</FormLabel>
            <FormControl>
              <Input
                disabled
      ...
      <div className="flex justify-end gap-3 max-w-4xl mx-auto border-t pt-4">
        <Button
          type="button"
          variant="outline"
          onClick={() => router.push(`/corporate-training/accounts/${account.id}`)}
        >
          Cancel
        </Button>

...
export default async function BatchReportPage() {
  ...
  const batches = await prisma.batch.findMany({
    ...
    include: {
      ...
      waitlist: { where: { isDeleted: false } },
    },
  });

  const totalBatches = batches.length;
  const totalWaitlist = batches.reduce(
    (sum, batch) =>
      sum + batch.waitlist.filter((entry) => entry.status === 'Waiting').length,
    0,
  );
  const totalPromoted = batches.reduce(
    (sum, batch) =>
      sum +
      batch.waitlist.filter((entry) => entry.status === 'Promoted').length,
    0,
  );
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-3">
        <StatCard
          title="Batches"
          value={totalBatches}
          description="Visible roster rows"
        ...
        <StatCard
          title="Promoted"
          value={totalPromoted}
          description="Moved from waiting"
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Current enrollment counts and waitlist activity by batch.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
            ...
            <TableBody>
              {batches.map((batch) => {
                const fillRate =
                  batch.capacity > 0
                    ? Math.round(
                        (batch.currentEnrollmentCount / batch.capacity) * 100,
                      )
                    : 0;
                return (

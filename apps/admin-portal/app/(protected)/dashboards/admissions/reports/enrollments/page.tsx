...
export default async function EnrollmentReportPage() {
  ...
  const pricingGlobal = await prisma.enrollment.count({
    where: {
      isDeleted: false,
      branchId: { in: branchIds },
  ...
  const pricingBatch = await prisma.enrollment.count({
    where: {
      isDeleted: false,
      branchId: { in: branchIds },
  ...
  const pricingBranch = await prisma.enrollment.count({
    where: {
      isDeleted: false,
      branchId: { in: branchIds },
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <StatCard
          ...
          value={paymentValidationCount}
          description="Enrollments needing fee validation"
          icon={<School className="h-5 w-5" />}
          tone="amber"
        />
        <StatCard
          title="Pricing Source Mix"
          value={`${pricingBatch}/${pricingBranch}/${pricingGlobal}`}
          description="Batch / branch / global"
        ...
        <StatCard
          ...
          description="Finished learning cycles"
          icon={<FileSpreadsheet className="h-5 w-5" />}
          tone="sky"
        />
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Enrollment states and pricing mix in the selected scope.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Most recent rows with pricing and batch links.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
                ...
                <TableRow key={enrollment.id}>
                  ...
                  <TableCell>
                    <div className="text-sm text-slate-600">
                      {enrollment.pricingSource}
                    </div>
                    <div className="text-xs text-slate-500">
                      {enrollment.paymentValidationRequired

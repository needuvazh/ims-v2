...
export async function GET(
  ...
  return withRouteObservability(request.headers, async () => {
    ...
    const rateLimit = withRateLimit(request, 200, 60_000, `/api/public/courses/${slug}`);
    ...
    try {
      ...
      logger.error('api.public.courses.detail.failed', { status: 'failed', error: error as Error });
      return problemJson(
        500,

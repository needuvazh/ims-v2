...
export async function GET(request: Request) {
  return withRouteObservability(request.headers, async () => withPermission(request, 'organization.manage', async () => {
    ...
    try {
      ...
      const result = await organizationService.listDepartments(parsed.data.branchId, {
      ...
      logger.error('api.organization.departments.failed', { status: 'failed', error: error as Error });
      return NextResponse.json(
        {
          ...
          errorCode: 'ORG-DEPARTMENTS-FAILED',
        },
        { status: 500 }
      );
    }

    ...
    async () =>
      withPermission(request, 'admission.create', async ({ session }) => {
        ...
        try {
          ...
          if (contentType.includes('multipart/form-data')) {
            ...
            if (!file || !documentType) {
              return NextResponse.json(
                {
                  success: false,
                  messageEnglish: 'file and documentType are required',
                },
                { status: 400 },
              );
            }
            ...
            if (!documentType || !fileKey) {
              return NextResponse.json(
                {
                  success: false,
                  messageEnglish: 'documentType and fileKey are required',
                },
                { status: 400 },
              );
            }
          ...
          logger.error('api.admissions.documents.register.failed', {
            status: 'failed',

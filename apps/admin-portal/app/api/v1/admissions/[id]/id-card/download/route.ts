...
export async function GET(request: Request, props: { params: Promise<{ id: string }> }) {
  ...
  return withRouteObservability(request.headers, async () => withPermission(request, 'admission.read', async ({ session }) => {
    ...
    try {
      ...
      const fullName = `${admission.person.firstName} ${admission.person.lastName}`.trim().toUpperCase();
      ...
      doc.setFontSize(5);
      doc.text(`Course: ${courseName.length > 28 ? courseName.substring(0, 25) + '...' : courseName}`, 26, 31);
      ...
      logger.error('api.admissions.idcard.download.failed', { status: 'failed', error: error as Error });

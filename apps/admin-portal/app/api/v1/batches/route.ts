...
export function batchErrorResponse(error: Error) {
  ...
  if (
    errCode === 'ERR_CRS_DUPLICATE_BATCH_CODE' ||
    msg.includes('ERR_CRS_DUPLICATE_BATCH_CODE')
  ) {
    status = 422;
    ...
    errCode === 'ERR_CRS_INVALID_DATE_RANGE' ||
    msg.includes('ERR_CRS_INVALID_DATE_RANGE')
  ) {
    status = 400;
    ...
    errCode === 'ERR_CRS_BATCH_NO_TRAINER' ||
    msg.includes('ERR_CRS_BATCH_NO_TRAINER')
  ) {
    status = 422;
    ...
    errCode === 'ERR_CRS_BATCH_FULL' ||
    msg.includes('ERR_CRS_BATCH_FULL')
  ) {
    status = 422;
    ...
    errCode === 'ERR_CRS_PRIMARY_TRAINER_ALREADY_ASSIGNED' ||
    msg.includes('ERR_CRS_PRIMARY_TRAINER_ALREADY_ASSIGNED')
  ) {
    status = 422;
    ...
    errCode === 'ERR_CRS_TRAINER_SCHEDULE_CONFLICT' ||
    msg.includes('ERR_CRS_TRAINER_SCHEDULE_CONFLICT')
  ) {
    status = 422;
    ...
    errCode === 'ERR_CRS_INVALID_STATE_TRANSITION' ||
    msg.includes('ERR_CRS_INVALID_STATE_TRANSITION')
  ) {
    status = 400;
    ...
    errCode === 'ERR_CRS_WALKIN_COMPLETION_NOT_ALLOWED' ||
    msg.includes('ERR_CRS_WALKIN_COMPLETION_NOT_ALLOWED')
  ) {
    status = 422;
    ...
    errCode === 'ERR_CRS_COURSE_NOT_PUBLISHED' ||
    msg.includes('ERR_CRS_COURSE_NOT_PUBLISHED')
  ) {
    status = 422;
    ...
    errCode === 'ERR_IAM_INSUFFICIENT_PERMISSIONS' ||
    msg.includes('ERR_IAM_INSUFFICIENT_PERMISSIONS')
  ) {
    status = 403;
    ...
    errCode === 'ERR_CRS_CLASSROOM_NOT_FOUND' ||
    msg.includes('ERR_CRS_CLASSROOM_NOT_FOUND')
  ) {
    status = 404;
    ...
    errCode === 'ERR_CRS_INVALID_CORPORATE_ACCOUNT' ||
    msg.includes('ERR_CRS_INVALID_CORPORATE_ACCOUNT')
  ) {
    status = 422;
    ...
    errCode === 'ERR_CRS_BATCH_NOT_FOUND' ||
    msg.includes('ERR_CRS_BATCH_NOT_FOUND')
  ) {
    status = 404;
    ...
    errCode === 'ERR_CRS_CONCURRENCY_VIOLATION' ||
    msg.includes('ERR_CRS_CONCURRENCY_VIOLATION')
  ) {
    status = 409;
    ...
    msg.includes('ERR_STUDENT_PROFILE_NOT_FOUND') ||
    msg.includes('ERR_CRS_STUDENT_NOT_FOUND') ||
    msg.includes('ERR_STUDENT_NOT_FOUND')
  ) {
    status = 404;
    ...
    messageEn =
      'Access denied: student profile is not authorized for this branch scope.';
    ...
    async () =>
      withPermission(request, 'course.catalog.view', async ({ session }) => {
        ...
        try {
          ...
          logger.error('api.batches.list.failed', {
            status: 'failed',
    ...
    async () =>
      withPermission(request, 'batch.delivery.create', async ({ session }) => {
        ...
        try {
          ...
          logger.error('api.batches.create.failed', {
            status: 'failed',
            error: error as Error,
          });
          return batchErrorResponse(error as Error);

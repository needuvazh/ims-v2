...
export async function POST(
  ...
  return withRouteObservability(request.headers, async () =>
    withPermission(request, 'batch.waitlist.manage', async ({ session }) => {
      ...
      const parsed = skipSchema.safeParse(payload);
      if (!parsed.success) {
        return problemJson(
          400,
          'Invalid request body',
          'Skip details are invalid.',
      ...
      try {
        const { waitlistId, reason } = parsed.data;
        ...
        const result = await batchService.skipWaitlistEntry(id, waitlistId, reason, session.userId);
        ...
        applyObservabilityResponseHeaders(response.headers, request.headers, {
          route: '/api/v1/batches/[id]/waitlist/skip',
        ...
        logger.error('api.batches.waitlist-skip.failed', { status: 'failed', error: error as Error });

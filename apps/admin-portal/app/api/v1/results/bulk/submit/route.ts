  ...
  return withRouteObservability(request.headers, async () =>
    withPermission(request, 'result.create', async ({ session }) => {
      ...
      try {
        ...
        const handler = new SubmitBulkResultsCommandHandler(resultRepository, examRepository);
        ...
        const response = NextResponse.json({ success: true, data: { processedCount: count } }, { status: 200 });
        ...
        logger.error('api.results.bulk.submit.failed', { status: 'failed', error: error as Error });

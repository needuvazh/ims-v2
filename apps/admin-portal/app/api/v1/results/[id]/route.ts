  ...
  return withRouteObservability(request.headers, async () =>
    withPermission(request, 'result.view', async ({ session }) => {
      ...
      try {
        ...
        const handler = new GetResultDetailQueryHandler(resultRepository, examRepository);

        const result = await handler.execute({ resultId: id });

        if (!result) {
          return NextResponse.json(
            { success: false, errorCode: 'RESULT_NOT_FOUND', messageEnglish: 'Result not found.', statusCode: 404 },
            { status: 404 },
          );
        }
        ...
        logger.error('api.results.detail.failed', { status: 'failed', error: error as Error });

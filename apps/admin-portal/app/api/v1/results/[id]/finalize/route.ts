  ...
  return withRouteObservability(request.headers, async () =>
    withPermission(request, 'result.finalize', async ({ session }) => {
      ...
      try {
        ...
        const parsed = FinalizeResultSchema.safeParse({ resultId: id, ...payload });
        if (!parsed.success) {
        ...
        const response = NextResponse.json({ success: true, data: { id, status: 'Finalized' } }, { status: 200 });
        ...
        logger.error('api.results.finalize.failed', { status: 'failed', error: error as Error });

  ...
  return withRouteObservability(request.headers, async () =>
    withPermission(request, 'result.view', async ({ session }) => {
      ...
      try {
        ...
        const result = await handler.execute({ examId, enrollmentId, status, page, pageSize });
        ...
        logger.error('api.results.list.failed', { status: 'failed', error: error as Error });
  ...
  return withRouteObservability(request.headers, async () =>
    withPermission(request, 'result.create', async ({ session }) => {
      ...
      try {
        ...
        return examResultProblemJson(400, 'Invalid request body', 'Request body must be valid JSON.', 'RESULT_INVALID_JSON');
      }
      ...
      try {
        ...
        const handler = new RecordResultCommandHandler(resultRepository, examRepository, enrollmentReader);
        ...
        const response = NextResponse.json({ success: true, data: { id: resultId } }, { status: 201 });
        applyObservabilityResponseHeaders(response.headers, request.headers, {
          route: '/api/v1/results',
          method: request.method,
          status: 'success',
        ...
        logger.error('api.results.create.failed', { status: 'failed', error: error as Error });
        return examResultErrorResponse(error as Error);
      }
    })
  , { route: '/api/v1/results' });

    ...
    async () =>
      withPermission(request, 'completion.view', async ({ session }) => {
        ...
        try {
          ...
          const handler = new SearchCompletionsQueryHandler(
          ...
          logger.error('api.completions.list.failed', {
            status: 'failed',
    ...
    async () =>
      withPermission(request, 'completion.evaluate', async ({ session }) => {
        ...
        try {
          ...
          const response = NextResponse.json(
            { success: true, data: { id: completionId } },
            { status: 201 },
          );
          applyObservabilityResponseHeaders(response.headers, request.headers, {
          ...
          logger.error('api.completions.evaluate.failed', {
            status: 'failed',
            error: error as Error,
          });
          return examResultErrorResponse(error as Error);

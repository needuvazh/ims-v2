...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  return withRouteObservability(request.headers, async () =>
    withPermission(request, 'completion.coordinator-review', async ({ session }) => {
      ...
      try {
        ...
        const handler = new CoordinatorReviewCommandHandler(completionRepository);
        ...
        logger.error('api.completions.coordinator-review.failed', { status: 'failed', error: error as Error });

    ...
    async () =>
      withPermission(request, 'completion.evaluate', async ({ session }) => {
        ...
        try {
          ...
          const parsed = ReevaluateCompletionSchema.safeParse({
            completionId: id,
            ...payload,
          });
          if (!parsed.success) {
          ...
          if (completion.completionStatus === 'Approved') {
            const reevalHandler = new ReevaluateCompletionCommandHandler(
              completionRepository,
            );
            await reevalHandler.execute({
              completionId: id,
          ...
          const response = NextResponse.json(
            { success: true, data: { id, status: finalStatus } },
            { status: 200 },
          );
          ...
          logger.error('api.completions.reevaluate.failed', {
            status: 'failed',

      ...
      try {
        ...
        if (!result) {
          return NextResponse.json(
            { success: false, errorCode: 'COMPLETION_NOT_FOUND', messageEnglish: 'Completion not found.', statusCode: 404 },
            { status: 404 },
          );
        }
        ...
        logger.error('api.completions.detail.failed', { status: 'failed', error: error as Error });

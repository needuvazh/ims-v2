...
function problemJson(
  ...
  return NextResponse.json(
    {
      type: 'https://ims.local/problems/export-job-download',
      title,
      status,
      detail,
      errorCode,
      ...
      try {
        ...
        if (!job.fileUrl)
          return problemJson(
            409,
            'Export not ready',
            'The export file is not available yet.',
            'IAM-EXPORT-NOT-READY',
          );

        const filePath = job.fileUrl.startsWith('file://')
          ? job.fileUrl.substring(7)
          : job.fileUrl;
        let fileBuffer: Buffer;
        try {
          fileBuffer = await fs.readFile(filePath);
        } catch (err) {
          logger.error('api.exportJobs.download.file_missing', {
            status: 'failed',
            error: err as Error,
            details: { filePath },
          } as any);
          return problemJson(
            ...
            'The exported file could not be found on the server.',
            'IAM-EXPORT-FILE-MISSING',
          );
        }
        ...
        logger.info('api.exportJobs.download.succeeded', {
          status: 'success',
        ...
        logger.error('api.exportJobs.download.failed', {
          status: 'failed',
        ...
        return problemJson(
          500,
          'Export download failed',
          'Unable to download export at this time.',
          'IAM-EXPORT-DOWNLOAD-FAILED',
        );
      }
    },
    { route: '/api/v1/reports/iam/export-jobs/:jobId/download' },

...
function problemJson(
  ...
  return NextResponse.json(
    {
      type: 'https://ims.local/problems/report',
      title,
      status,
      detail,
      errorCode,
    ...
    try {
      ...
      if (reportType === 'user-directory') {
        const filters: UserListFilters = {
          ...
          status: parsed.data.status as UserListFilters['status'],
        };
        data = await userService.searchUsers(
          filters,
          parsed.data.page,
        ...
        data = await loginHistoryQueryService.listSecurityLoginHistory(
          {
            branchId: parsed.data.branchId,
            status:
              reportType === 'failed-logins' ? 'Failure' : parsed.data.status,
          },
          parsed.data.page,
          parsed.data.pageSize,
          {
        ...
        data = await userService.searchUsers(
          { branchId: parsed.data.branchId, status: 'Locked' },
          parsed.data.page,
          parsed.data.pageSize,
          {
            actorId: session.userId,
        ...
        data = await roleService.listRoles(
          parsed.data.page,
          parsed.data.pageSize,
          {
            actorId: session.userId,
        ...
        data = await auditQueryService.listAuditLogs(
          { branchId: parsed.data.branchId, module: 'iam' },
          parsed.data.page,
          parsed.data.pageSize,
          {
            actorId: session.userId,
      ...
      applyObservabilityResponseHeaders(response.headers, request.headers, {
        route: `/api/v1/reports/iam/${reportType}`,
        method: request.method,
        status: 'success',
      });
      logger.info('api.reports.get.succeeded', {
        status: 'success',
        reportType,
      });
      return response;
      ...
      logger.error('api.reports.get.failed', {
        status: 'failed',

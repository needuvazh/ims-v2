...
function problemJson(
  ...
  return NextResponse.json(
    {
      type: 'https://ims.local/problems/report-export',
      title,
      status,
      detail,
      errorCode,
  ...
  return withRouteObservability(request.headers, async () => {
    ...
    if (!parsed.success) {
      return problemJson(
        ...
        'IAM-VAL-REPORT-EXPORT-INVALID_BODY',
    ...
    try {
      ...
      applyObservabilityResponseHeaders(response.headers, request.headers, {
        route: `/api/v1/reports/iam/${reportType}/export`,
        method: request.method,
        status: 'success',
      });
      logger.info('api.reports.export.succeeded', {
        status: 'success',
        reportType,
        exportJobId: job.id,
      });
      ...
      logger.error('api.reports.export.failed', {
        status: 'failed',
      ...
      return problemJson(
        500,
        'Report export failed',
        'Unable to create export at this time.',
        'IAM-REPORT-EXPORT-FAILED',
      );
    }
  });
}

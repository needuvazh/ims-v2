...
function problemJson(
  ...
  return NextResponse.json(
    {
      type: 'https://ims.local/problems/security-policy',
      title,
      status,
      detail,
      errorCode,
  ...
  return withRouteObservability(
    ...
    async () => {
      ...
      try {
        ...
        logger.error('api.security.policy.get.failed', {
          status: 'failed',
        ...
        return problemJson(
          500,
          'Security policy failed',
          'Unable to load the security policy at this time.',
          'IAM-SECURITY-POLICY-FAILED',
        );
      }
    },
    { route: '/api/v1/security/policy' },
    ...
    async () => {
      ...
      if (!parsed.success) {
        return problemJson(
          400,
          'Invalid request body',
          'Security policy details are invalid.',
          'IAM-VAL-SECURITY-INVALID_BODY',
        );
      }
      ...
      try {
        ...
        logger.info('api.security.policy.update.succeeded', {
          status: 'success',
        });
        return response;
      } catch (error) {
        ...
        logger.error('api.security.policy.update.failed', {
          status: 'failed',
          error: error as Error,
        });
        return problemJson(
          500,
          'Security policy update failed',
          'Unable to update the security policy at this time.',
          'IAM-SECURITY-POLICY-UPDATE-FAILED',
        );
      }
    },
    { route: '/api/v1/security/policy' },

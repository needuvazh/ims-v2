...
function problemJson(
  ...
  return NextResponse.json(
    {
      type: 'https://ims.local/problems/login-history',
      title,
      status,
      detail,
      errorCode,
  ...
  return withRouteObservability(
    ...
    async () => {
      ...
      try {
        ...
        logger.info('api.security.loginHistory.succeeded', {
          status: 'success',
        });
        return response;
      } catch (error) {
        ...
        logger.error('api.security.loginHistory.failed', {
          status: 'failed',
        ...
        return problemJson(
          500,
          'Login history failed',
          'Unable to load login history at this time.',
          'IAM-LOGIN-HISTORY-FAILED',
        );
      }
    },
    { route: '/api/v1/security/login-history' },

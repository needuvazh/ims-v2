    ...
    async () =>
      withPermission(request, 'document.view', async ({ session }) => {
        ...
        try {
          ...
          const hasAccess = await service.verifyDocumentAccess(
            session.userId,
            id,
          );
          if (!hasAccess) {
          ...
          logger.error('api.documents.get.failed', {
            status: 'failed',
        ...
        try {
          ...
          if (docRecord && docRecord.fileKey.startsWith('http')) {
            try {
              ...
              logger.warn('api.documents.delete.vercel_blob_failed', {
                error: blobErr as Error,
              });
            }
          }
          ...
          logger.error('api.documents.delete.failed', {
            status: 'failed',
            error: error as Error,
          });
          return documentErrorResponse(error as Error);

...
function crmErrorResponse(error: Error) {
  ...
  if (msg.includes('ERR_CRM_LEAD_NOT_FOUND')) {
    ...
    messageEn =
      'Lost reason code and descriptive notes (min 15 characters) are mandatory.';
...
export async function POST(
  ...
  return withRouteObservability(
    request.headers,
    async () =>
      withPermission(request, 'lead.lost', async ({ session }) => {
        ...
        try {
          ...
          const result = await leadService.closeLeadLost(
            leadId,
            parsed.data,
            session.userId,
          );
          ...
          logger.error('api.crm.leads.lost.failed', {
            status: 'failed',

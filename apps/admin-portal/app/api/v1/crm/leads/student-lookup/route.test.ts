...
describe('Lead student lookup API route', () => {
  ...
  it('GET /api/v1/crm/leads/student-lookup returns no match when no student profile exists', async () => {
    withPermissionMock.mockImplementation((_req, _perm, cb) =>
      cb({ session: { userId: 'user-1', permissions: ['lead.create'], activeBranchId: '11111111-1111-1111-1111-111111111111' } })
    );
    ...
    const response = await GET(
      new Request('http://localhost/api/v1/crm/leads/student-lookup?email=fatima@example.com', {
        method: 'GET',
      })
    );
  ...
  it('GET /api/v1/crm/leads/student-lookup returns matched student details when profile exists', async () => {
    ...
    globalPersonLookupMock.mockResolvedValue({
      ...
      preflight: { hasActiveAdmission: false, activeAdmissionId: null, hasEnrollment: true, conflictCode: null },
    });

    const { GET } = await import('./route');
    const response = await GET(
      new Request('http://localhost/api/v1/crm/leads/student-lookup?mobile=96899112233', {
        method: 'GET',
      })
    );
    ...
    expect(globalPersonLookupMock).toHaveBeenCalledWith('96899112233', null, { revealSensitive: false });
  });
});

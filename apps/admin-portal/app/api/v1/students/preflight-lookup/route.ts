  ...
  return withRouteObservability(request.headers, async () =>
    withPermission(request, 'student.create', async ({ session }) => {
      ...
      try {
        ...
        const activeBranchId = parsed.data.branchId ?? session.activeBranchId ?? '';
        ...
        const query = parsed.data.email ?? parsed.data.mobile ?? parsed.data.nationalId ?? '';
        const result = await studentQueryService.globalPersonLookup(query, activeBranchId, { revealSensitive: false });
        ...
        if ((error as Error).message === 'ERR_AUTH_BRANCH_DENIED') {
          return NextResponse.json(
            { success: false, errorCode: 'ERR_AUTH_BRANCH_DENIED', messageEnglish: 'Branch scope denied.', statusCode: 403 },
            { status: 403 }
          );
        }
        return NextResponse.json(

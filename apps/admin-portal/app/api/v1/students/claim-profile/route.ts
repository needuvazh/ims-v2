  ...
  return withRouteObservability(
    ...
    async () =>
      withPermission(request, 'student.create', async ({ session }) => {
        ...
        try {
          const { otpService, admissionService, branchScopeResolver, prisma } =
          ...
          if (
            !allowedBranches.some(
              (branchId) => branchId === parsed.data.branchId,
            )
          ) {
            return NextResponse.json(
              {
          ...
          if (!valid) {
            return NextResponse.json(
              {
                success: false,
                errorCode: 'ERR_STU_OTP_INVALID',
                messageEnglish:
                  'The OTP is invalid or has expired. Please request a new code.',
                statusCode: 422,
              },
              { status: 422 },
            );
          ...
          if (existingAdmission > 0) {
            return NextResponse.json(
              {
                success: false,
                errorCode: 'ERR_ADM_ACTIVE_ADMISSION_EXISTS',
                messageEnglish:
                  'This student already has an active admission at the target branch.',
                statusCode: 409,
              },
              { status: 409 },
            );
          ...
          logger.error('api.students.claim-profile.failed', {
            status: 'failed',

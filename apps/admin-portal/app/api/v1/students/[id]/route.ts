...
const updateStudentSchema = z.object({
  ...
  dateOfBirth: z.string().trim().regex(/^\d{4}-\d{2}-\d{2}$/).optional().nullable(),
...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  return withRouteObservability(request.headers, async () => {
    ...
    try {
      ...
      if (!hasPermission(session, 'student.update') && !hasPermission(session, 'student.write')) {
        ...
        return NextResponse.json(
          {
            ...
            messageEnglish: parsed.error.issues[0]?.message || 'Invalid payload.',
            statusCode: 400,
          },
          { status: 400 }
        );
      ...
      if (parsed.data.version !== undefined && parsed.data.version !== student.version) {
        return NextResponse.json(
          {
            success: false,
            errorCode: 'ERR_STU_CONCURRENCY_CONFLICT',
            messageEnglish: 'The student profile was updated by another user. Please reload and try again.',
            statusCode: 409,
          },
          { status: 409 }
        );
      ...
      const updated = await prisma.$transaction(async (tx) => {
        const person = await tx.person.update({
          ...
          data: {
            ...
            ...(parsed.data.passportNumber !== undefined ? { passportNumber: parsed.data.passportNumber } : {}),
            ...(parsed.data.visaNumber !== undefined ? { visaNumber: parsed.data.visaNumber } : {}),
      ...
      logger.error('api.students.update.failed', { status: 'failed', error: error as Error });
      return NextResponse.json(
        {
          success: false,

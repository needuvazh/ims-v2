...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  return withRouteObservability(request.headers, async () =>
    withPermission(request, 'student.archive', async ({ session }) => {
      ...
      try {
        ...
        await studentStatusService.archive({ studentProfileId, actorId: session.userId, branchId: profile.branchId, reason: 'Archived via student module' });
        const response = NextResponse.json({ success: true, data: { studentProfileId, status: 'Archived' } }, { status: 200 });
        applyObservabilityResponseHeaders(response.headers, request.headers, { route: '/api/v1/students/[id]/archive', method: request.method, status: 'success' });
        return response;
      } catch (error) {
        logger.error('api.students.archive.failed', { status: 'failed', error: error as Error });
    ...
    { route: '/api/v1/students/[id]/archive' }
  );
}

    ...
    async () =>
      withPermission(request, 'student.archive', async ({ session }) => {
        ...
        try {
          ...
          await studentStatusService.archive({
            studentProfileId,
            actorId: session.userId,
            branchId: profile.branchId,
            reason: 'Archived via student module',
          });
          const response = NextResponse.json(
            { success: true, data: { studentProfileId, status: 'Archived' } },
            { status: 200 },
          );
          ...
          logger.error('api.students.archive.failed', {
            status: 'failed',

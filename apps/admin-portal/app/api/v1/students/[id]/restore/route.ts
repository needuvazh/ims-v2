  ...
  return withRouteObservability(request.headers, async () =>
    withPermission(request, 'student.restore', async ({ session }) => {
      ...
      try {
        ...
        const response = NextResponse.json({ success: true, data: { studentProfileId, status: 'Active' } }, { status: 200 });
        applyObservabilityResponseHeaders(response.headers, request.headers, { route: '/api/v1/students/[id]/restore', method: request.method, status: 'success' });
        return response;
      } catch (error) {
        logger.error('api.students.restore.failed', { status: 'failed', error: error as Error });

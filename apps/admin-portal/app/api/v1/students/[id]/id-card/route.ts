...
export async function POST(
  ...
  return withRouteObservability(request.headers, async () => {
    ...
    try {
      ...
      if (!hasPermission(session, 'student.idcard.manage') && !hasPermission(session, 'student.id_card.issue')) {
        return NextResponse.json(
          { success: false, errorCode: 'ERR_AUTH_PERMISSION_DENIED', messageEnglish: 'Missing student.idcard.manage permission.', statusCode: 403 },
          { status: 403 }
        );
      }
      ...
      const profile = await prisma.studentProfile.findUnique({
        ...
        select: { id: true, branchId: true, idCardIssued: true, idCardNumber: true, isDeleted: true },
      });
      ...
      if (!allowedBranches.some((branchId) => branchId === profile.branchId)) {
        return NextResponse.json(
          { success: false, errorCode: 'ERR_AUTH_BRANCH_SCOPE_DENIED', messageEnglish: 'Student profile is outside your allowed branch scope.', statusCode: 403 },
          { status: 403 }
        );
      }

...
export async function POST(request: Request, props: { params: Promise<{ id: string }> }) {
  ...
  return withRouteObservability(request.headers, async () => withPermission(request, 'student.reveal_pii', async ({ session }) => {
    ...
    try {
        ...
        return NextResponse.json(
          {
            ...
            messageEnglish: 'Invalid reveal parameters. Reason must be at least 5 chars.',
            statusCode: 400,
          },
          { status: 400 }
        );
      ...
      logger.error('api.students.reveal-pii.failed', { status: 'failed', error: error as Error });
      ...
      } else if ((error as Error).message === 'ERR_STUDENT_PROFILE_NOT_FOUND' || (error as Error).message === 'ERR_STUDENT_NOT_FOUND') {
        status = 404;

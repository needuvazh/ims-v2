...
export async function POST(
  ...
  const { id: studentProfileId } = await props.params;
  return withRouteObservability(
    request.headers,
    async () =>
      withPermission(request, 'student.write', async ({ session }) => {
        ...
        try {
          ...
          if (!profile || profile.isDeleted) {
            return NextResponse.json(
              { success: false, messageEnglish: 'Student profile not found' },
              { status: 404 },
            );
          }
          ...
          if (!allowedBranches.includes(profile.branchId as Uuid)) {
            return NextResponse.json(
          ...
          const pathname = `students/${profile.personId}/profile-photo/${file.name}`;
          ...
          applyObservabilityResponseHeaders(response.headers, request.headers, {
            route: '/api/v1/students/[id]/profile-photo',
            method: request.method,
            status: 'success',
          });
          return response;
        } catch (error) {
          logger.error('api.students.profile-photo.failed', {
            status: 'failed',

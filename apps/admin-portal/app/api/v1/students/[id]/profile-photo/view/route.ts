    ...
    async () =>
      withPermission(request, 'student.read', async ({ session }) => {
        ...
        try {
          ...
          if (!profile || !profile.person.photoUrl) {
          ...
          const blobResponse = await fetch(profile.person.photoUrl, {
            headers: {
          ...
          applyObservabilityResponseHeaders(response.headers, request.headers, {
            route: '/api/v1/students/[id]/profile-photo/view',
            method: request.method,
            status: 'success',
          });
          return response;
        } catch (error) {
          logger.error('api.students.profile-photo.view.failed', {
            status: 'failed',

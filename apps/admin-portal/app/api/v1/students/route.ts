...
export async function POST(request: Request) {
  return withRouteObservability(request.headers, async () => {
    ...
    try {
      ...
      if (!hasPermission(session, 'student.create') && !hasPermission(session, 'student.write')) {
        return NextResponse.json(
          {
            success: false,
      ...
      const identityConflict = await prisma.person.findFirst({
        where: {
          OR: [
            ...
            ...(parsed.data.email ? [{ email: parsed.data.email, isDeleted: false }] : []),
            ...(parsed.data.nationalId ? [{ nationalId: parsed.data.nationalId, isDeleted: false }] : []),
            ...(parsed.data.passportNumber ? [{ passportNumber: parsed.data.passportNumber, isDeleted: false }] : []),
            ...(parsed.data.visaNumber ? [{ visaNumber: parsed.data.visaNumber, isDeleted: false }] : []),
          ],
        },
        select: { id: true },
      });
      ...
      logger.error('api.students.create.failed', { status: 'failed', error: error as Error });
      return NextResponse.json(
        {
          success: false,
          errorCode: 'ERR_STUDENT_INTERNAL_ERROR',

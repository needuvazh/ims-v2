...
const bodySchema = z.object({
  survivorStudentProfileId: z.string().uuid('survivorStudentProfileId must be a UUID.'),
  sourceStudentProfileId: z.string().uuid('sourceStudentProfileId must be a UUID.'),
  mergeReason: z.string().min(10, 'mergeReason must be at least 10 characters.').max(500),
});
...
export async function POST(request: Request) {
  return withRouteObservability(request.headers, async () =>
    withPermission(request, 'student.merge', async ({ session }) => {
      ...
      try {
        const { studentMergeService } = await import('../../../../../lib/runtime');
        ...
        if (msg === 'ERR_STU_MERGE_SELF_FORBIDDEN') {
          return NextResponse.json(
            { success: false, errorCode: 'ERR_STU_MERGE_SELF_FORBIDDEN', messageEnglish: 'Survivor and source profiles cannot be the same.', statusCode: 422 },
            { status: 422 }
          );
        }
        ...
        if (msg.startsWith('ERR_STU_MERGE_')) {
          return NextResponse.json(
            { success: false, errorCode: msg, messageEnglish: msg, statusCode: 404 },
            { status: 404 }
          );
        }

        logger.error('api.students.merge.failed', { status: 'failed', error: error as Error });
        return NextResponse.json(
          { success: false, errorCode: 'ERR_STUDENT_INTERNAL_ERROR', messageEnglish: msg, statusCode: 500 },
          { status: 500 }
        );
      }
    }),

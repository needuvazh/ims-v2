  ...
  return withRouteObservability(
    ...
    async () =>
      withPermission(request, 'course.catalog.create', async ({ session }) => {
        ...
        try {
          const { courseCompletionRuleService } =
          ...
          const result = await courseCompletionRuleService.createCompletionRule(
            ruleInput,
            session.userId,
          );
          ...
          logger.error('api.courses.completion-rules.create.failed', {
            status: 'failed',
          ...
          if (msg.includes('ERR_CRS_COURSE_NOT_FOUND')) {
            ...
            messageEn =
              'Overlapping active completion rule already exists for this course.';
          }
    ...
    async () =>
      withPermission(request, 'course.catalog.view', async () => {
        ...
        try {
          ...
          logger.error('api.courses.completion-rules.list.failed', {
            status: 'failed',
            error: error as Error,
          });
          return NextResponse.json(
      ...
      withPermission(request, 'course.catalog.create', async ({ session }) => {
        ...
        try {
          ...
          if (parsed.data.action === 'disable') {
            const result =
              await courseCompletionRuleService.disableCompletionRule(
                parsed.data.id,
                session.userId,
              );
            const response = NextResponse.json(
          ...
          return problemJson(
            ...
            'CRS-VAL-RULES-UNSUPPORTED_ACTION',
          );
        } catch (error) {
          logger.error('api.courses.completion-rules.patch.failed', {
            status: 'failed',
            error: error as Error,
          });
          const msg = (error as Error).message;

    ...
    async () =>
      withPermission(request, 'course.catalog.view', async () => {
        ...
        try {
          ...
          const evaluationDate = parsed.data.asOfDate
            ? parseDateOnly(parsed.data.asOfDate)
            ...
            return NextResponse.json(
              {
                ...
                messageEnglish:
                  'Active completion rule not found for the specified course and date.',
                statusCode: 404,
              },
              { status: 404 },
            );
          ...
          logger.error('api.courses.completion-rules.active.failed', {
            status: 'failed',

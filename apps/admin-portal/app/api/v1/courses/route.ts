...
export function courseCatalogErrorResponse(error: Error) {
  ...
  if (msg.includes('ERR_CRS_DUPLICATE_CODE')) {
    ...
    messageAr =
      'يجب أن يكون رمز الدورة بحروف وأرقام إنجليزية كبيرة بين 3 و 20 حرفاً.';
    ...
    messageAr =
      'يجب تكوين قاعدة تسعير وقاعدة إكمال نشطة واحدة على الأقل لنشر الدورة.';
    ...
    messageEn =
      'Arabic name or description must contain only Arabic characters.';
    ...
    async () =>
      withPermission(request, 'course.catalog.view', async () => {
        ...
        try {
          ...
          logger.error('api.courses.list.failed', {
            status: 'failed',
    ...
    async () =>
      withPermission(request, 'course.catalog.create', async ({ session }) => {
        ...
        try {
          ...
          logger.error('api.courses.create.failed', {
            status: 'failed',
            error: error as Error,
          });
          return courseCatalogErrorResponse(error as Error);

...
function problemJson(
  ...
  return NextResponse.json(
    {
      type: 'https://ims.local/problems/dashboard',
      title,
      status,
      detail,
      errorCode,
  ...
  return withRouteObservability(request.headers, async () => {
    ...
    if (!requiredPermission) {
      return problemJson(
        404,
        'Dashboard not found',
        'Requested dashboard is not supported.',
        'IAM-SYS-001',
      );
    }
    ...
    try {
      ...
      const users = await userService.searchUsers({}, 1, 1000, {
        actorId: session.userId,
        actorPermissions: session.permissions,
        activeBranchId: session.activeBranchId,
      });
      const roles = await roleService.listRoles(1, 1000, {
        actorId: session.userId,
        actorPermissions: session.permissions,
        activeBranchId: session.activeBranchId,
      });
      const permissionsList = await permissionService.searchPermissions(
        undefined,
        undefined,
        {
          actorId: session.userId,
      ...
      const audit = await auditQueryService.listAuditLogs(
        { module: 'iam' },
        1,
        1,
        {
      ...
      const data =
        dashboardType === 'security'
          ? {
              ...
              lockedAccounts: users.items.filter(
          ...
          : dashboardType === 'admin'
            ? {
                ...
                activeUsers: users.items.filter(
      ...
      applyObservabilityResponseHeaders(response.headers, request.headers, {
        route: `/api/v1/dashboards/iam/${dashboardType}`,
        method: request.method,
        status: 'success',
      });
      logger.info('api.dashboards.get.succeeded', {
        status: 'success',
        dashboardType,
      });
      return response;
      ...
      logger.error('api.dashboards.get.failed', {
        status: 'failed',
      ...
      return problemJson(
        500,
        'Dashboard failed',
        'Unable to load the dashboard at this time.',
        'IAM-DASHBOARD-FAILED',
      );
    }
  });
}

...
function problemJson(
  ...
  return NextResponse.json(
    {
      type: 'https://ims.local/problems/role-users',
      title,
      status,
      detail,
      errorCode,
  ...
  return withRouteObservability(
    ...
    async () => {
      ...
      try {
        ...
        logger.info('api.roles.users.list.succeeded', {
          status: 'success',
          roleId: params.id,
        });
        return response;
        ...
        logger.error('api.roles.users.list.failed', {
          status: 'failed',
        ...
        return problemJson(
          500,
          'Role users failed',
          'Unable to load role users at this time.',
          'IAM-ROLE-USERS-FAILED',
        );
      }
    },
    { route: '/api/v1/roles/:id/users' },

...
function problemJson(
  ...
  return NextResponse.json(
    {
      type: 'https://ims.local/problems/role-detail',
      title,
      status,
      detail,
      errorCode,
  ...
  return withRouteObservability(
    ...
    async () => {
      ...
      try {
        ...
        const result = await roleService.listRoles(1, 1000, {
          actorId: session.userId,
          actorPermissions: session.permissions,
          activeBranchId: session.activeBranchId,
        });
        const role = result.items.find(
        ...
        logger.info('api.roles.get.succeeded', {
          status: 'success',
          roleId: params.id,
        });
        return response;
        ...
        logger.error('api.roles.get.failed', {
          status: 'failed',
        ...
        return problemJson(
          500,
          'Role detail failed',
          'Unable to load role at this time.',
          'IAM-ROLES-GET-FAILED',
        );
      }
    },
    { route: '/api/v1/roles/:id' },
  ...
  return withRouteObservability(
    ...
    async () => {
      ...
      try {
        ...
        logger.info('api.roles.update.succeeded', {
          status: 'success',
          roleId: params.id,
        });
        return response;
        ...
        logger.error('api.roles.update.failed', {
          status: 'failed',
          error: error as Error,
          roleId: params.id,
        });

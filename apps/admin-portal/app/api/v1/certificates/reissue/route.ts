      ...
      withPermission(request, 'certificate.reissue', async ({ session }) => {
        ...
        try {
          ...
          const reissueRequests =
            await prisma.certificateReissueRequest.findMany({
              where,
              include: {
                certificate: {
                  include: {
          ...
          logger.error('api.certificates.reissue.list.failed', {
            status: 'failed',
    ...
    async () =>
      withPermission(request, 'certificate.reissue', async ({ session }) => {
        ...
        try {
          ...
          const response = NextResponse.json(
            { success: true, data: { id: requestId } },
            { status: 201 },
          );
          applyObservabilityResponseHeaders(response.headers, request.headers, {
            route: '/api/v1/certificates/reissue',
          ...
          logger.error('api.certificates.reissue.create.failed', {
            status: 'failed',
            error: error as Error,
          });
          return certificateErrorResponse(error as Error);

  ...
  return withRouteObservability(request.headers, async () =>
    withPermission(request, 'certificate.view', async ({ session }) => {
      ...
      try {
        ...
        logger.error('api.certificates.list.failed', { status: 'failed', error: error as Error });
  ...
  return withRouteObservability(request.headers, async () =>
    withPermission(request, 'certificate.create', async ({ session }) => {
      ...
      try {
        ...
        const certificateId = await service.execute(parsed.data, session.userId);
        ...
        logger.error('api.certificates.generate.failed', { status: 'failed', error: error as Error });
        return certificateErrorResponse(error as Error);
      }
    })
  , { route: '/api/v1/certificates' });

  ...
  return withRouteObservability(request.headers, async () =>
    withPermission(request, 'exam.update', async ({ session }) => {
      ...
      try {
        ...
        const handler = new ActivateExamCommandHandler(examRepository);

        await handler.execute({ examId: id, userId: session.userId });

        const response = NextResponse.json({ success: true, data: { id, status: 'OpenForResultEntry' } }, { status: 200 });
        applyObservabilityResponseHeaders(response.headers, request.headers, {
          route: `/api/v1/exams/${id}/activate`,
          method: request.method,
          status: 'success',
        });
        return response;
      } catch (error) {
        logger.error('api.exams.activate.failed', { status: 'failed', error: error as Error });

...
describe('users route', () => {
  ...
  it('returns 403 when permission is missing', async () => {
    const { NextResponse } = await import('next/server');
    withPermissionMock.mockResolvedValue(
      NextResponse.json({ errorCode: 'FORBIDDEN' }, { status: 403 }),
    );
  ...
  it('returns invalid field error for malformed email on create user', async () => {
    ...
    expect(
      body.invalidFields?.some(
        (field: { field: string }) => field.field === 'email',
      ),
    ).toBe(true);
  });
  ...
  it('returns forbidden when branch scope is bypassed', async () => {
    ...
    const response = await GET(
      new Request(
        'http://localhost/api/v1/users?branchId=22222222-2222-2222-2222-222222222222',
      ),
    );

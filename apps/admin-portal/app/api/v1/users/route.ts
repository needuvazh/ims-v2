...
const querySchema = z.object({
  ...
  status: z
    .enum(['PendingActivation', 'Active', 'Locked', 'Suspended', 'Archived'])
    .optional(),
    ...
    async () =>
      withPermission(request, 'iam.user.read', async ({ session }) => {
        ...
        try {
          ...
          logger.info('api.users.list.succeeded', {
            status: 'success',
            page: parsed.data.page,
            pageSize: parsed.data.pageSize,
          });
          ...
          logger.error('api.users.list.failed', {
            status: 'failed',
...
export async function POST(request: Request) {
  return withRouteObservability(
    request.headers,
    async () =>
      withPermission(request, 'iam.user.create', async ({ session }) => {
        const logger = createStructuredLogger(getCurrentRequestContext() ?? {});
        ...
        try {
          ...
          logger.info('api.users.create.succeeded', {
            status: 'success',
            userId: user.id,
          });
          return response;
          ...
          logger.error('api.users.create.failed', {
            status: 'failed',
            error: error as Error,
          });
          return problemJson(
            500,
            'User create failed',
            'Unable to create the user at this time.',
            'IAM-USERS-CREATE-FAILED',
          );
        }
      }),
    { route: '/api/v1/users' },

...
function problemJson(
  ...
  return NextResponse.json(
    {
      type: 'https://ims.local/problems/user-archive',
      title,
      status,
      detail,
      errorCode,
    ...
    async () => {
      ...
      if (!parsed.success) {
        return problemJson(
          ...
          'Archive reason is invalid.',
          'IAM-VAL-USER-ARCHIVE-INVALID_BODY',
      ...
      try {
        ...
        await userService.archiveUser(params.id, {
        ...
        logger.info('api.users.archive.succeeded', {
          status: 'success',
          userId: params.id,
        });
        return response;
        ...
        logger.error('api.users.archive.failed', {
          status: 'failed',
        ...
        return problemJson(
          500,
          'User archive failed',
          'Unable to archive user at this time.',
          'IAM-USER-ARCHIVE-FAILED',
        );
      }
    },
    { route: '/api/v1/users/:id/archive' },

...
vi.mock('../../../../../../lib/runtime', () => ({
  userService: { archiveUser: archiveUserMock },
}));
...
describe('user archive route', () => {
  ...
  it('archives a user when authorized', async () => {
    assertPermissionMock.mockResolvedValue({
      userId: 'user-1',
      permissions: ['iam.user.archive'],
    ...
    const response = await POST(
      new Request('http://localhost/api/v1/users/user-1/archive', {
        method: 'POST',
      }),
      { params: { id: 'user-1' } },
    );

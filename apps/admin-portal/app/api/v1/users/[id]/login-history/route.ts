...
function problemJson(
  ...
  return NextResponse.json(
    {
      type: 'https://ims.local/problems/user-login-history',
      title,
      status,
      detail,
      errorCode,
      ...
      try {
        ...
        if (!parsed.success) {
          return problemJson(
            ...
            'IAM-VAL-LOGIN-HISTORY-INVALID_QUERY',
        ...
        const { loginHistoryQueryService } =
        ...
        logger.info('api.users.loginHistory.succeeded', {
          status: 'success',
          userId: params.id,
        });
        return response;
        ...
        logger.error('api.users.loginHistory.failed', {
          status: 'failed',
        ...
        return problemJson(
          500,
          'User login history failed',
          'Unable to load user login history at this time.',
          'IAM-USER-LOGIN-HISTORY-FAILED',
        );
      }
    },
    { route: '/api/v1/users/:id/login-history' },

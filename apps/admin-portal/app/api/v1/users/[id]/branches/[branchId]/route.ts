...
function problemJson(
  ...
  return NextResponse.json(
    {
      type: 'https://ims.local/problems/user-branch-remove',
      title,
      status,
      detail,
      errorCode,
  ...
  return withRouteObservability(
    ...
    async () => {
      ...
      try {
        ...
        await branchAccessService.removeBranchFromUser(
          params.id,
          params.branchId,
          null,
          {
        ...
        logger.info('api.users.branches.remove.succeeded', {
          status: 'success',
          userId: params.id,
        ...
        logger.error('api.users.branches.remove.failed', {
          status: 'failed',
        ...
        return problemJson(
          500,
          'Branch removal failed',
          'Unable to remove branch at this time.',
          'IAM-USER-BRANCH-REMOVE-FAILED',
        );
      }
    },
    { route: '/api/v1/users/:id/branches/:branchId' },

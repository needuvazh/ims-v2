...
function problemJson(
  ...
  return NextResponse.json(
    {
      type: 'https://ims.local/problems/user-suspend',
      title,
      status,
      detail,
      errorCode,
    ...
    async () => {
      ...
      if (!parsed.success) {
        return problemJson(
          ...
          'IAM-VAL-USER-SUSPEND-INVALID_BODY',
      ...
      try {
        ...
        await userService.suspendUser(params.id, {
        ...
        logger.info('api.users.suspend.succeeded', {
          status: 'success',
          userId: params.id,
        });
        return response;
        ...
        logger.error('api.users.suspend.failed', {
          status: 'failed',
        ...
        return problemJson(
          500,
          'User suspension failed',
          'Unable to suspend user at this time.',
          'IAM-USER-SUSPEND-FAILED',
        );
      }
    },
    { route: '/api/v1/users/:id/suspend' },

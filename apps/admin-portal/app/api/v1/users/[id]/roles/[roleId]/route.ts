...
function problemJson(
  ...
  return NextResponse.json(
    {
      type: 'https://ims.local/problems/user-role-remove',
      title,
      status,
      detail,
      errorCode,
    ...
    async () => {
      ...
      if (!parsed.success) {
        return problemJson(
          ...
          'Role revocation reason is invalid.',
      ...
      try {
        ...
        logger.info('api.users.roles.remove.succeeded', {
          status: 'success',
          userId: params.id,
        ...
        logger.error('api.users.roles.remove.failed', {
          status: 'failed',
        ...
        return problemJson(
          500,
          'Role revocation failed',
          'Unable to revoke role at this time.',
          'IAM-USER-ROLE-REMOVE-FAILED',
        );
      }
    },
    { route: '/api/v1/users/:id/roles/:roleId' },

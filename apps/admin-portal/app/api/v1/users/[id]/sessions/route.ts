...
function problemJson(
  ...
  return NextResponse.json(
    {
      type: 'https://ims.local/problems/user-sessions',
      title,
      status,
      detail,
      errorCode,
  ...
  return withRouteObservability(
    ...
    async () => {
      ...
      try {
        ...
        logger.info('api.users.sessions.list.succeeded', {
          status: 'success',
          userId: params.id,
        });
        return response;
        ...
        logger.error('api.users.sessions.list.failed', {
          status: 'failed',
  ...
  return withRouteObservability(
    ...
    async () => {
      ...
      try {
        ...
        logger.info('api.users.sessions.terminateAll.succeeded', {
          status: 'success',
          userId: params.id,
        });
        return response;
        ...
        logger.error('api.users.sessions.terminateAll.failed', {
          status: 'failed',
          error: error as Error,
          userId: params.id,
        });
        return problemJson(
          500,
          'Terminate all sessions failed',
          'Unable to terminate sessions at this time.',
          'IAM-USER-SESSIONS-TERMINATE-FAILED',
        );
      }
    },
    { route: '/api/v1/users/:id/sessions' },

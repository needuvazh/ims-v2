...
describe('scheduling calendar routes', () => {
  beforeEach(() => {
    ...
    withPermissionMock.mockImplementation((_req, _perm, cb) =>
      cb({ session: { userId: 'user-1', activeBranchId: 'branch-1' } }),
    );
  });

  it('POST /api/v1/scheduling/calendars creates a business calendar', async () => {
    createBusinessCalendarMock.mockResolvedValue({
      id: 'calendar-1',
      name: '2026 Calendar',
    });
    ...
    const response = await POST(
      new Request('http://localhost/api/v1/scheduling/calendars', {
        method: 'POST',
  ...
  it('GET /api/v1/scheduling/calendars returns calendars', async () => {
    ...
    const response = await GET(
      new Request('http://localhost/api/v1/scheduling/calendars?year=2026'),
    );

    const body = await response.json();
    expect(response.status).toBe(200);
    expect(body.success).toBe(true);
    expect(listCalendarsMock).toHaveBeenCalledWith(

...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  return withRouteObservability(request.headers, async () => withPermission(request, 'scheduling.calendar.update', async ({ session }) => {
    ...
    try {
      ...
      return problemJson('https://ims.local/problems/scheduling-branch-override-create', 400, 'Branch override create failed', 'Request body must be valid JSON.', 'SCH-CAL-INVALID-JSON');
    }

    const parsed = createBranchOverrideSchema.safeParse({ ...(payload as Record<string, unknown>), businessCalendarId: id });
    if (!parsed.success) {
      return problemJson(
    ...
    try {
      ...
      const created = await schedulingCalendarService.createBranchOverride(parsed.data, {
      ...
      logger.error('api.scheduling.branch-overrides.create.failed', { status: 'failed', error: error as Error, entityId: id, entityType: 'BusinessCalendar' });
      return problemJson(
        'https://ims.local/problems/scheduling-branch-override-create',
        400,
        'Branch override create failed',

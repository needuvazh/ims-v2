...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  return withRouteObservability(request.headers, async () => withPermission(request, 'scheduling.calendar.read', async () => {
    ...
    try {
      ...
      const resolved = await schedulingCalendarService.resolveCalendar(branchId, parsed.data.date ?? new Date(), parsed.data.instituteId);

      const response = NextResponse.json({ success: true, data: resolved }, { status: 200 });
      ...
      logger.error('api.scheduling.calendars.resolved.failed', { status: 'failed', error: error as Error, entityId: id, entityType: 'BusinessCalendar' });
      return problemJson(
        'https://ims.local/problems/scheduling-calendar-resolved',
        400,
        'Resolved calendar lookup failed',

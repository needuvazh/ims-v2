          ...
          try {
            const updated =
              parsed.data.status === 'Active'
                ? await schedulingCalendarService.activateBusinessCalendar(
                    id,
                    parsed.data.version,
                    { actorId: session.userId },
                  )
                : parsed.data.status === 'Closed'
                  ? await schedulingCalendarService.closeBusinessCalendar(
                      id,
                      parsed.data.version,
                      { actorId: session.userId },
                    )
                  : await schedulingCalendarService.archiveBusinessCalendar(
                      id,
                      parsed.data.version,
                      { actorId: session.userId },
                    );
            ...
            logger.error('api.scheduling.calendars.status.failed', {
              status: 'failed',

...
vi.mock('../../../../lib/runtime', () => ({
  branchAccessService: { switchActiveBranch: switchBranchMock },
}));
...
describe('auth switch branch route', () => {
  ...
  it('returns forbidden for unassigned branch', async () => {
    ...
    switchBranchMock.mockRejectedValue(
      new IamError(
        'IAM-AUTHZ-002',
        403,
        'Branch access denied',
        'تم رفض الوصول إلى الفرع',
      ),
    );

    const { POST } = await import('./route');
    const response = await POST(
      new Request('http://localhost/api/v1/auth/switch-branch', {
        method: 'POST',

    ...
    async () => {
      ...
      if (!token) {
        return problemJson(
          ...
          'IAM-AUTH-REFRESH-MISSING_TOKEN',
        );
      }

      try {
        const { authService, effectivePermissionsService } =
        ...
        response.cookies.set(accessTokenCookieName, result.accessToken, {
          ...
          maxAge: Math.max(
            0,
            Math.floor(
              (result.accessTokenExpiresAt.getTime() - Date.now()) / 1000,
            ),
          ),
        });
        ...
        logger.error('api.auth.refresh.failed', {
          status: 'failed',
        ...
        return problemJson(
          500,
          'Refresh failed',
          'Unable to refresh the session at this time.',
          'IAM-AUTH-REFRESH-FAILED',
        );
      }
    },
    { route: '/api/v1/auth/refresh' },

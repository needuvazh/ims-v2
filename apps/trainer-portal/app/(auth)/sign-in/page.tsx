    ...
    <div className="flex min-h-screen w-full font-sans bg-slate-50 overflow-hidden">
      ...
      >
        ...
        <div className="absolute inset-0 bg-slate-900">
          <img src="https://images.unsplash.com/photo-1544717302-de2939b7ef71?q=80&w=800&auto=format&fit=crop" className="absolute inset-0 h-full w-full object-cover mix-blend-luminosity opacity-30" alt="" />
          <div className="absolute inset-0 bg-gradient-to-br from-cyan-600/80 via-blue-600/80 to-indigo-800/60" />
        </div>

        {/* Animated blobs */}
        <motion.div animate={{ scale: [1, 1.2, 1], rotate: 360 }} transition={{ duration: 40, repeat: Infinity, ease: 'linear' }} className="absolute -top-[20%] -left-[10%] w-[800px] h-[800px] rounded-full bg-emerald-400/30 blur-[100px]" />
        ...
        <div className="relative z-10 flex h-full flex-col justify-between p-12 xl:p-16 text-white">
          ...
          <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1 }} className="flex items-center gap-4">
             <div className="flex -space-x-3">
               {[1,2,3].map(i => (
                 <img key={i} src={`https://i.pravatar.cc/100?img=${i+20}`} className="w-10 h-10 rounded-full border-2 border-cyan-500 object-cover" alt="Trainer" />
               ))}
             </div>
        ...
        <div className="flex flex-1 items-center justify-center px-6 py-10 sm:px-12">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.4 }} className="w-full max-w-[400px]">
            ...
            <div className="text-center mb-10">
              <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring", delay: 0.6 }} className="w-16 h-16 mx-auto bg-cyan-100 rounded-3xl flex items-center justify-center mb-6 shadow-inner border border-cyan-200 -rotate-3">
                <ClipboardCheck className="w-8 h-8 text-cyan-600" />
              </motion.div>
              <h1 className="text-3xl font-black text-slate-900 mb-2">Trainer Login</h1>
              <p className="text-slate-500 text-sm">Secure access to your teaching tools.</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative group">
                  ...
                  <input
                    type="email" placeholder="trainer@example.com" required
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between ml-1">
                  ...
                  <a href="#" className="text-xs font-bold text-cyan-600 hover:text-blue-600 transition-colors">Forgot?</a>
                </div>
                <div className="relative group">
                  <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                  ...
                  />
                  <button type="button" onClick={() => setShowPass(!showPass)} className="absolute right-3 top-1/2 -translate-y-1/2 p-2 rounded-xl text-slate-400 hover:text-cyan-600 hover:bg-cyan-50 transition-colors">
              ...
              <motion.button
                ...
                type="submit"
                disabled={isPending}
                className="w-full rounded-2xl bg-gradient-to-r from-cyan-500 to-blue-600 py-4 text-sm font-bold text-white shadow-lg shadow-cyan-500/30 transition-all disabled:opacity-70 flex items-center justify-center gap-2 mt-6"
              >
                {isPending ? (

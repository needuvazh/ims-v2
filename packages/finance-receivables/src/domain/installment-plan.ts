...
export function validateInstallmentPlan(input: CreateInstallmentPlanInput): void {
  ...
  if (input.installments.length !== input.numberOfInstallments) {
    throw new Error(`Number of installments (${input.installments.length}) does not match numberOfInstallments field (${input.numberOfInstallments})`);
  }
  ...
  if (!sum.equals(expectedTotal)) {
    throw new Error(`Sum of installment amounts (${sum.toString()}) does not match total amount of the plan (${expectedTotal.toString()})`);
  }

  // Validate sequence numbers are continuous starting from 1
  const sortedSeqs = [...input.installments].map(i => i.sequenceNumber).sort((a, b) => a - b);
  for (let idx = 0; idx < sortedSeqs.length; idx++) {
    if (sortedSeqs[idx] !== idx + 1) {
      throw new Error(`Sequence numbers must be consecutive and start from 1. Found sequence: ${sortedSeqs.join(', ')}`);
    }
  }
}

...
test('createInvoice should compute line totals, subtotal, tax, final total and persist invoice with initial open receivable', async () => {
  ...
  vi.spyOn(service['repo'], 'getNextInvoiceNumber').mockResolvedValue(
    'INV-2026-000001',
  );
  vi.spyOn(service['repo'], 'createInvoice').mockResolvedValue(
    mockInvoice as any,
  );
  ...
  expect(service['repo'].getNextInvoiceNumber).toHaveBeenCalledWith(
    'branch-123',
    mockPrisma,
  );
...
test('createInvoice should create an installment plan when subCategory is Installment', async () => {
  ...
  vi.spyOn(service['repo'], 'upsertReceivable').mockResolvedValue(null as any);
  vi.spyOn(service['repo'], 'createInstallmentPlan').mockResolvedValue(
    mockInstallmentPlan as any,
  );
...
test('createInstallmentPlan should fail validation if total amount does not match sum of installment amounts', async () => {
  ...
  await expect(service.createInstallmentPlan(input)).rejects.toThrow(
    'Sum of installment amounts (290) does not match total amount of the plan (300)',
  );
});
...
test('recordPayment should auto-allocate to invoice directly and generate receipt, transitioning status to Paid', async () => {
  ...
  expect(service['repo'].updateInvoiceStatus).toHaveBeenCalledWith(
    'invoice-123',
    'Paid',
    100,
    0,
...
test('verifyCorporateCreditLimit should throw if outstanding amount exceeds limit and block limit flag is true', async () => {
  ...
  await expect(
    service.verifyCorporateCreditLimit('corp-123', 'branch-123', 100),
  ).rejects.toThrow(
    'Corporate credit limit exceeded. Available credit is 50.000 OMR but requested 100.000 OMR.',
  );

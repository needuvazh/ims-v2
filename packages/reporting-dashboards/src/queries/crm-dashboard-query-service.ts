...
export class CrmDashboardQueryService {
  ...
  async getCrmDashboardData(
    ...
    widgets.push({
      ...
      ariaLabel:
        'Bar chart showing lead count distribution across different operational stages.',
      description:
        'Breakdown of current leads by their active lifecycle stage.',
    });
    ...
    widgets.push({
      ...
      description:
        'Percentage of total leads successfully converted to student admissions.',
    });
    ...
    widgets.push({
      ...
      description:
        'Distribution of leads categorized by lead acquisition channels.',
    });
    ...
    if (userContext.permissions.includes('REPORTING_VIEW_COUNSELOR_METRICS')) {
      const counselorPerformance =
        await this.leadAnalyticsReadService.getCounselorPerformance(
          userContext,
        );
      widgets.push({
        ...
        ariaLabel:
          'Bar chart showing total number of leads converted by counselor.',
        description: 'Converted leads comparison across active counselors.',
      });
    }

    // 7. Fetch total leads vs targets
    const leadsVsTargets =
      await this.leadAnalyticsReadService.getTotalLeadsVsTargets(userContext);
    widgets.push({
      id: 'leads-vs-targets',
      ...
      data: {
        ...
        progressPercentage:
          leadsVsTargets.target > 0
            ? Math.min(
                Math.round(
                  (leadsVsTargets.actual / leadsVsTargets.target) * 100,
                ),
                100,
              )
            : 0,
      ...
      description:
        'Overall active lead count compared against branch/counselor target goals.',
    });

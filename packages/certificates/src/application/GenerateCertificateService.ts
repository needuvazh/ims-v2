  ...
  async execute(
    ...
    if (!context) {
      throw new DomainError(
        ...
        `Enrollment ${validated.enrollmentId} not found`,
      );
    }

    // 2. Check branch-scoping if requestedBranchId is provided
    if (
      validated.requestedBranchId &&
      context.branchId !== validated.requestedBranchId
    ) {
      throw new DomainError(
        ErrorCodes.BRANCH_SCOPE_DENIED,
        'Access denied for this branch',
      );
    }

    // 3. Verify completion approval
    const isCompleted = await this.completionReadPort.isCompletionApproved(
      validated.enrollmentId,
    );
    if (!isCompleted) {
      throw new DomainError(
    ...
    if (context.paymentValidationRequired) {
      const isPaid = await this.financeValidationPort.isPaymentValidationPassed(
        validated.enrollmentId,
      );
      if (!isPaid) {
        throw new DomainError(

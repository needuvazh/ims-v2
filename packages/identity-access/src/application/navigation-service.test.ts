...
describe('navigation service', () => {
  it('filters nav by permissions and resolves user labels', () => {
    ...
    expect(resolvePortalNavigation('admin', session).map((item) => item.href)).toContain('/organization/institutes');
    expect(resolvePortalNavigation('admin', session).map((item) => item.href)).toContain('/iam/users');

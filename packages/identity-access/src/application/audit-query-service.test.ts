...
describe('AuditQueryService', () => {
  ...
  const scopedLog = createAuditLog({
    id: 'audit-1' as never,
    branchId: '11111111-1111-1111-1111-111111111111' as never,
    entityId: 'user-1',
  });
  const otherBranchLog = createAuditLog({
    id: 'audit-2' as never,
    branchId: '22222222-2222-2222-2222-222222222222' as never,
    entityId: 'user-2',
  });
    ...
    repo = {
      ...
      list: async (filters, page, pageSize) => {
        const items = Array.from(logs.values()).filter((entry) => {
          ...
          if (filters.performerId && entry.performedBy !== filters.performerId)
            return false;
          if (filters.module && entry.module !== filters.module) return false;
          if (filters.startDate && entry.performedAt < filters.startDate)
            return false;
          if (filters.endDate && entry.performedAt > filters.endDate)
            return false;
          return true;
        });
        return {
  ...
  it('lists audit logs with branch scoping and permission', async () => {
    const result = await service.listAuditLogs({}, 1, 20, {
      ...
      actorPermissions: ['iam.audit.read'],
      activeBranchId: '11111111-1111-1111-1111-111111111111' as never,
    });
  ...
  it('returns an audit log by id within branch scope', async () => {
    const result = await service.getAuditLogById(scopedLog.id as never, {
      actorId: crypto.randomUUID() as never,
      actorPermissions: ['iam.audit.read'],
      activeBranchId: '11111111-1111-1111-1111-111111111111' as never,
    });
  ...
  it('rejects audit log access outside branch scope', async () => {
    await expect(
      service.getAuditLogById(otherBranchLog.id as never, {
        actorId: crypto.randomUUID() as never,
        actorPermissions: ['iam.audit.read'],
        activeBranchId: '11111111-1111-1111-1111-111111111111' as never,
      }),

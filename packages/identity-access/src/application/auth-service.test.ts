...
describe('AuthService', () => {
  ...
  const resetTokens = new Map<
    string,
    UserActivationTokenDto & { usedAt: Date | null }
  >();
  ...
  beforeEach(async () => {
    process.env.SESSION_SECRET =
      'auth-service-test-secret-auth-service-test-secret';
    ...
    userRepo = {
      ...
      findPersonById: async (id) =>
        id === currentPerson.id ? currentPerson : null,
      findPersonByMobile: async (mobile) =>
        mobile === currentPerson.mobile ? currentPerson : null,
      create: async (user) => {
    ...
    sessionRepo = {
      ...
      findByHashedRefreshToken: async (hash) =>
        Array.from(sessions.values()).find(
          (session) =>
            session.hashedRefreshToken === hash ||
            session.previousHashedRefreshToken === hash,
        ) ?? null,
    ...
    activationTokenRepo = {
      create: async (token) => {
        resetTokens.set(token.tokenHash, {
          ...token,
  ...
  it('signs in an active user and records login history and audit', async () => {
    ...
    expect(
      auditRepo
        .list()
        .some((entry) => entry.action === 'iam.user.login-succeeded'),
    ).toBe(true);
  });
  ...
  it('locks the account after repeated invalid passwords', async () => {
    for (let i = 0; i < 5; i++) {
      await expect(
        ...
        errorCode: i < 4 ? 'IAM-AUTH-001' : 'IAM-AUTH-002',
      });
    }
    ...
    expect(
      auditRepo.list().some((entry) => entry.action === 'iam.user.locked'),
    ).toBe(true);
  });
  ...
  it('rejects suspended users', async () => {
    ...
    ).rejects.toMatchObject({ errorCode: 'IAM-AUTH-003' });
  });
  ...
  it('rejects archived users and SQL injection style emails', async () => {
    ...
    ).rejects.toMatchObject({ errorCode: 'IAM-AUTH-001' });
  ...
  it('rejects expired passwords', async () => {
    ...
    ).rejects.toMatchObject({ errorCode: 'IAM-AUTH-004' });
  });
  ...
  it('expires the oldest active session when the concurrent limit is reached', async () => {
    ...
    expect(
      auditRepo
        .list()
        .some((entry) => entry.action === 'iam.session.expired-by-policy'),
    ).toBe(true);
  });
  ...
  it('logs out by revoking the session', async () => {
    ...
    expect(
      auditRepo.list().some((entry) => entry.action === 'iam.user.logout'),
    ).toBe(true);
  });
  ...
  it('creates a password reset token and notification intent', async () => {
    ...
    expect(
      auditRepo
        .list()
        .some((entry) => entry.action === 'iam.user.password-reset-requested'),
    ).toBe(true);
  });
  ...
  it('resets the password with a valid token and revokes active sessions', async () => {
    ...
    sessions.set(
      's1',
      createSession({ accessTokenJti: 's1', userId: currentUser.id }),
    );

    await authService.resetPassword('reset-token', 'NewPassword@123!');

    expect(
      await passwordPolicy.verify(currentPasswordHash, 'NewPassword@123!'),
    ).toBe(true);
    ...
    expect(
      auditRepo
        .list()
        .some((entry) => entry.action === 'iam.user.password-reset-completed'),
    ).toBe(true);
  });
  ...
  it('rejects weak or reused reset passwords', async () => {
    ...
    await expect(
      authService.resetPassword('reset-token-2', 'short'),
    ).rejects.toMatchObject({ errorCode: 'IAM-VAL-005' });
    await expect(
      authService.resetPassword('reset-token-2', 'Password@123'),
    ).rejects.toMatchObject({ errorCode: 'IAM-VAL-009' });
  });

  it('rejects expired reset tokens and invalid refresh tokens', async () => {
    const expiredHash = crypto
      .createHash('sha256')
      .update('expired-reset')
      .digest('hex');
    resetTokens.set(expiredHash, {
      id: crypto.randomUUID(),
      userId: currentUser.id,
    ...
    await expect(
      authService.resetPassword('expired-reset', 'NewPassword@123!'),
  ...
  it('changes the password for an authenticated user', async () => {
    ...
    await authService.changePassword(
      ...
      signIn.session as any,
    );

    expect(
      auditRepo
        .list()
        .some((entry) => entry.action === 'iam.user.password-changed'),
    ).toBe(true);
  });
  ...
  it('rejects password change with wrong current password', async () => {
    ...
    await expect(
      authService.changePassword(
        { currentPassword: 'WrongPassword!', newPassword: 'NewPassword@123!' },
        signIn.session as any,
      ),
    ).rejects.toMatchObject({ errorCode: 'IAM-AUTH-001' });
  });

...
describe('SessionService', () => {
  ...
  beforeEach(() => {
    ...
    sessions.set(
      crypto.randomUUID(),
      createSession({ userId: userId as never }),
    );
  ...
  it('lists active sessions with permission', async () => {
    const result = await service.listUserSessions(userId as never, {
      actorId: crypto.randomUUID() as never,
      actorPermissions: ['iam.session.read'],
      activeBranchId: null,
    });
  ...
  it('terminates a session and records audit', async () => {
    await service.terminateSession(sessionId as never, {
      actorId: crypto.randomUUID() as never,
      actorPermissions: ['iam.session.terminate'],
      activeBranchId: null,
    });
    ...
    expect(
      auditRepo
        .list()
        .some((entry) => entry.action === 'iam.session.terminated'),
    ).toBe(true);
  });

  it('terminates all sessions for a user and records audit', async () => {
    await service.terminateAllUserSessions(userId as never, {
      actorId: crypto.randomUUID() as never,
      actorPermissions: ['iam.session.terminate'],
      activeBranchId: null,
    });
    ...
    expect(
      Array.from(sessions.values()).every(
        (session) => session.userId !== userId || session.status === 'Revoked',
      ),
    ).toBe(true);
    expect(
      auditRepo
        .list()
        .some((entry) => entry.action === 'iam.session.all-terminated'),
    ).toBe(true);
  });

  it('rejects missing permission', async () => {
    await expect(
      service.terminateSession(sessionId as never, {
        actorId: crypto.randomUUID() as never,
        actorPermissions: [],
        activeBranchId: null,
      }),

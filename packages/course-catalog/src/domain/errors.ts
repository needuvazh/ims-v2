...
export class DuplicateCourseName extends DomainError {
  constructor(
    message = 'A course with this name already exists in this department.',
  ) {
...
export class InvalidCodeFormat extends DomainError {
  constructor(
    message = 'Course code must be uppercase alphanumeric and between 3 to 20 characters.',
  ) {
...
export class InvalidCategoryHierarchy extends DomainError {
  constructor(
    message = 'Cyclic parent-child hierarchy detected in categories.',
  ) {
...
export class ActiveCourseLocked extends DomainError {
  constructor(
    message = 'Duration or classification cannot be changed on a published course with active batches.',
  ) {
...
export class MissingPricingOrRules extends DomainError {
  constructor(
    message = 'A course must have at least one active pricing rule and one active completion rule configured to be published.',
  ) {
...
export class ActiveBatchesExist extends DomainError {
  constructor(
    message = 'Cannot archive course with active batches in OpenForEnrollment or InProgress status.',
  ) {

...
export class ReevaluateCompletionCommandHandler {
  ...
  async execute(input: ReevaluateCompletionInput): Promise<void> {
    ...
    if (completion.completionStatus !== COMPLETION_STATUSES.APPROVED) {
      throw new CompletionInvalidStateError(`Completion must be approved before reevaluation (status: ${completion.completionStatus})`);
    }

    if (completion.evidenceStale) {
      throw new CompletionEvidenceStaleError('Cannot reevaluate completion with stale evidence');
    }

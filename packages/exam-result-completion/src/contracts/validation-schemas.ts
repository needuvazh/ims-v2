...
export const CreateExamSchemaBase = z.object({
  ...
  examName: z.string().trim().min(3, 'Exam name must be at least 3 characters').max(200, 'Exam name must be at most 200 characters'),
  examDate: z.coerce.date({ message: 'Invalid exam date' }),
  maxMarks: z.coerce.number().positive('Max marks must be greater than 0').max(99999999.99, 'Max marks exceeds maximum allowed value'),
  passMarks: z.coerce.number().min(0, 'Pass marks must be >= 0'),
});

export const CreateExamSchema = CreateExamSchemaBase.refine((data) => data.passMarks <= data.maxMarks, {
  message: 'Pass marks must be <= max marks',
  path: ['passMarks'],
});

export const UpdateExamSchema = CreateExamSchemaBase.partial().omit({ courseId: true, batchId: true }).refine((data) => {
...
export const RecordResultSchema = z.object({
  ...
  marksObtained: z.coerce.number().min(0, 'Marks must be >= 0').max(99999999.99, 'Marks exceeds maximum allowed value'),
  grade: z.string().trim().max(20, 'Grade must be at most 20 characters').optional(),
});
...
export const BulkResultValidationSchema = z.object({
  examId: z.string().uuid('Invalid exam ID'),
  results: z.array(BulkResultRowSchema).min(1, 'At least one result is required').max(500, 'Maximum 500 results per batch'),
});
...
export const SearchExamsQuerySchema = z.object({
  ...
  status: z.enum(['Draft', 'Scheduled', 'OpenForResultEntry', 'Closed', 'Cancelled', 'Archived']).optional(),

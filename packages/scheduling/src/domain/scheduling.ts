...
export const operatingDaySchema = z
  ...
  .refine(
    ...
    {
      message:
        'Open days require working hours and closed days must not include working hours',
    },
  );
...
export const updateBusinessCalendarSchema = createBusinessCalendarSchema
  .partial()
  .omit({ instituteId: true, code: true });
...
export const updateBranchOverrideSchema = createBranchOverrideSchema
  .partial()
  .omit({ businessCalendarId: true, branchId: true, year: true });
...
const createVenueBlockSchemaRefined = createVenueBlockSchema.refine(
  (data) => {
  ...
  {
    message:
      'Venue block end date must be on or after the start date, and partial-day blocks require valid start and end times.',
  },
);

export { createVenueBlockSchemaRefined as createVenueBlockRefinedSchema };

export const updateVenueBlockSchema = createVenueBlockSchema
  .partial()
  .omit({ branchId: true });

export type CreateBusinessCalendarCommand = z.infer<
  typeof createBusinessCalendarSchema
>;
export type UpdateBusinessCalendarCommand = z.infer<
  typeof updateBusinessCalendarSchema
>;
export type CreateBranchOverrideCommand = z.infer<
  typeof createBranchOverrideSchema
>;
export type UpdateBranchOverrideCommand = z.infer<
  typeof updateBranchOverrideSchema
>;

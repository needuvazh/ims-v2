...
export class InMemoryOrganizationRepository implements OrganizationRepository {
  ...
  async findInstituteById(id: string) {
    return this.institutes.find((i) => i.id === id) ?? null;
  }
  async findInstituteByCode(instituteCode: string) {
    return (
      this.institutes.find((i) => i.instituteCode === instituteCode) ?? null
    );
  }
  async updateInstitute(id: string, updates: Partial<Institute>) {
    this.institutes = this.institutes.map((i) =>
      i.id === id ? { ...i, ...updates } : i,
    );
  ...
  async listInstitutes(
    ...
    return {
      items,
      total: this.institutes.length,
      page,
      pageSize,
      totalPages: Math.ceil(this.institutes.length / pageSize),
    };
  }
  ...
  async findBranchByCode(branchCode: string) {
    return this.branches.find((b) => b.branchCode === branchCode) ?? null;
  }
  ...
  async listBranches(
    ...
    if (filters?.instituteId)
      items = items.filter((b) => b.instituteId === filters.instituteId);
    if (filters?.status)
  ...
  // Department
  async createDepartment(input: Department) {
    this.departments = [...this.departments, input];
    return input;
  }
  ...
  async findDepartmentByCode(branchId: string, departmentCode: string) {
    return (
      this.departments.find(
        (d) => d.branchId === branchId && d.departmentCode === departmentCode,
      ) ?? null
    );
  }
  ...
  async listDepartments(
    branchId: string,
    filters?: { status?: Department['status'] },
  ) {
  ...
  // Classroom
  async createClassroom(input: Classroom) {
    this.classrooms = [...this.classrooms, input];
    return input;
  }
  ...
  async findClassroomByName(branchId: string, classroomName: string) {
    return (
      this.classrooms.find(
        (c) => c.branchId === branchId && c.classroomName === classroomName,
      ) ?? null
    );
  }
  ...
  async listClassrooms(
    ...
    if (filters?.branchId)
      items = items.filter((c) => c.branchId === filters.branchId);
    const page = filters?.page ?? 1;
    const pageSize = filters?.pageSize ?? 20;
    const paged = items.slice((page - 1) * pageSize, page * pageSize);
    return {
  ...
  async getOrganizationHierarchy(
    ...
    const instBranches = this.branches.filter(
      (b) => b.instituteId === instituteId,
    );

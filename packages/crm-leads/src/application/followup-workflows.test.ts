...
describe('FollowUpApplicationService - Workflows & Scheduling', () => {
  ...
  describe('scheduleFollowUp', () => {
    ...
    it('should schedule follow-up and denormalize nextFollowUpDate on active lead', async () => {
      ...
      followUpRepositoryMock.create.mockResolvedValue({
        id: 'followup-1',
        followUpDate: new Date(futureDate),
        followUpType: 'Call',
        status: 'Scheduled',
      });
      ...
      expect(leadRepositoryMock.updateStage).toHaveBeenCalledWith(
        'lead-1',
        'FollowUp',
        1,
        prismaMock,
      ...
      expect(leadRepositoryMock.updateLead).toHaveBeenCalledWith(
        'lead-1',
        {
  ...
  describe('recordOutcome', () => {
    it('should verify optimistic concurrency version', async () => {
      ...
      leadRepositoryMock.updateLead.mockRejectedValue(
        new Error('ERR_CRM_CONCURRENCY_VIOLATION'),
      );

      await expect(
        followUpService.recordOutcome(
          'followup-1',
          {
            outcome: 'Busy',
    ...
    it('should recalculate nextFollowUpDate to the next earliest scheduled date', async () => {
      ...
      await followUpService.recordOutcome(
        'followup-1',
        {
          outcome: 'Busy',

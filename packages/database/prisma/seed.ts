...
async function seed() {
  ...
  const rolesToCreate = [
    ...
    },
    {
      code: 'BRANCH_MANAGER',
      name: 'Branch Manager',
      desc: 'Branch-level administrative',
    },
    {
      code: 'COUNSELOR',
    ...
    },
    {
      code: 'FINANCE',
      name: 'Finance',
      desc: 'Finance and Billing Access',
    },
    {
      code: 'TRAINER',
      name: 'Trainer',
  ...
  const financePerms = permRecords.filter((p) =>
    financePermCodes.includes(p.permissionCode),
  );
  for (const perm of financePerms) {
    await prisma.rolePermission.create({
      data: { roleId: roleMap['FINANCE'].id, permissionId: perm.id },
    });
  }
  console.log(`  ✓ Assigned permissions to FINANCE`);
  ...
  const usersToSeed = [
    ...
    },
    {
      ...
      email: 'branchmanager@alsaud-intl.com',
      username: 'branchmanager@alsaud-intl.com',
    ...
    },
    {
      firstName: 'Al saud',
      lastName: 'Finance',
      email: 'finance@alsaud-intl.com',
      username: 'finance@alsaud-intl.com',
      userType: 'Finance',
      roleCode: 'FINANCE',
      phone: '+968 9219 0901',
      password: passwordHash,

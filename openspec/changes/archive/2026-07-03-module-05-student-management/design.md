...
- **Performance Risk:** Dynamic visibility checks require joining `admissions` and `enrollments` tables.
  - *Mitigation:* We have added database indexes on `StudentProfile(person_id)`, `StudentProfile(branch_id, student_status)`, `Admission(student_profile_id)`, and `Enrollment(student_profile_id)`.
- **OTP Delivery Failure:** If the student's phone number or email is incorrect, the OTP cannot be received.
  - *Mitigation:* An administrator can update the student's contact details at the origin branch first after manual ID check.

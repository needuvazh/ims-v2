...
- **Duplication during Inquiry Promoted to Lead:**
  - _Mitigation:_ The `Person` resolution logic queries by exact email or phone number match before inserting a new `Person` record, avoiding duplicate profiles.
- **Optimistic Locking Failures under High Concurrency:**
  - _Mitigation:_ Updates verify the `version` column. If a concurrency clash is detected, the transaction rolls back and returns `ERR_CRM_CONCURRENCY_VIOLATION` (HTTP 409), prompting the user to refresh their view.
- **Performance of Scoped Queries:**
  - _Mitigation:_ Heavy indexes are created on `branchId`, `counselorId`, `stage`, `status`, and `isDeleted` to ensure efficient query execution.

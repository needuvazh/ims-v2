...
- [x] 1.1 Ensure `schema.prisma` has `WalkInEnrollment` and `WalkInConfirmation` definitions (Verified).
- [x] 1.2 Run database migrations if any model changes are needed.

## 2. Application Logic

- [x] 2.1 Implement `createWalkInEnrollment` (Intake Orchestrator) in `EnrollmentService`:
...
- [x] 2.2 Implement `recordWalkInPayment` (Payment Command) in `EnrollmentService`:
...
- [x] 2.3 Reject `enrollmentType = 'WalkIn'` in the generic `createEnrollment` flow.

## 3. API & Controller Routing

- [x] 3.1 Expose `POST /api/v1/enrollments/walk-in` (Intake endpoint).
  - Enforce permission `enrollment.create`.
  - Enforce branch scoping (user's active branch).
- [x] 3.2 Expose `POST /api/v1/enrollments/{id}/walk-in-payment` (Payment recording endpoint).
  - Enforce permission `enrollment.record_payment`.
  - Enforce branch scoping (user's active branch).
- [x] 3.3 Reject requests with `enrollmentType = 'WalkIn'` in the generic route `POST /api/v1/enrollments`, returning `400 Bad Request` (`ERR_ENR_GENERIC_WALKIN_BLOCKED`).

## 4. Tests

- [x] 4.1 Write integration tests in `packages/admissions-enrollment/src/application/enrollment-lifecycle.test.ts` for `createWalkInEnrollment` and `recordWalkInPayment`:
...
- [x] 4.2 Verify the application compiles and passes all checks.

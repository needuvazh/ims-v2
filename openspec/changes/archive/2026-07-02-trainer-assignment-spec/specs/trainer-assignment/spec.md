...
The system SHALL support allocating active qualified trainers to batch deliveries within the execution start and end dates, subject to `batch.delivery.assign` permission and active branch authorization.
...
- **THEN** the system SHALL reject the assignment with `403 Forbidden`.

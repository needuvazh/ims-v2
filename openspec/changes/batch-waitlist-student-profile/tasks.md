...
- [ ] 1.2 Database Migration & Data Backfill:
...
- [ ] 1.4 Refactor Codebase Contracts for Rename:
...
- [ ] 2.1 Enforce Candidate Branch Scoping:
...
- [ ] 2.2 Enforce Active Status Checks:
...
- [ ] 2.3 Enforce Batch Status Invariants:
...
- [ ] 3.2 Implement Promotion Bypass Check:
...
- [ ] 3.4 Enforce Enrollment Uniqueness Invariant:
...
- [ ] 5.1 Unit Tests: Add/update unit tests in `packages/training-delivery` checking candidate validations, branch scoping, and status invariants.
- [ ] 5.2 Concurrency Tests: Write integration tests verifying that promoted waitlist candidates hold their seats and cannot be front-run by other enrollment approvals.
- [ ] 5.3 Worker Integration Tests: Verify that the worker successfully listens to `WaitlistEntryPromoted`, transitions the enrollment to `Approved`, and marks the waitlist record as `Removed`.
- [ ] 5.4 Run verification checks and verify all tests pass.

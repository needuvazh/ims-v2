...
- **THEN** The system MUST validate lead-facing Won preconditions: email and phone are valid, interested course is active, birthdate (`dateOfBirth` on `Person`) is not null, and at least one active document of type `CIVIL_ID_FRONT` or `PASSPORT_SCAN` is uploaded.
...
- **AND** The system MUST reject the conversion with error code `ERR_ADM_ACTIVE_ADMISSION_EXISTS` when the student already has an active admission (`Draft` or `Submitted`) in the target branch.

# crm-portal-ui-scoped-filtering Specification

## Purpose

TBD - created by archiving change crm-portal-ui-scoped-filtering. Update Purpose after archive.
...
The system SHALL warn users if a lead or inquiry is captured with a duplicate mobile number, email, or ID Number (nationalId), but SHALL allow authorized users to explicitly acknowledge and bypass the block during submission.

#### Scenario: Acknowledging and saving duplicate lead

- **WHEN** a user submits a lead with a phone number, email, or ID Number matching an existing record, receives the duplicate alert, checks the "Ignore duplicate warning and proceed" checkbox, and submits again
...
To ensure reliable duplicate detection on partial updates (e.g. PATCH calls), the system MUST merge the update properties with the existing lead details from the database (for branch, phone, email, nationalId, and course fields) before performing the 30-day active duplicate check.

#### Scenario: Partial update causing duplicate collision

- **WHEN** a counselor updates only the `phone` or `nationalId` field of a lead, and the merged state (same branch and course) matches an active lead from the past 30 days
- **THEN** the system triggers `ERR_CRM_DUPLICATE_LEAD_DETECTED` unless bypassed

### Requirement: CRM Existing Student Lookup

To prevent duplicate profile creation and prompt the counselor, the system SHALL check for existing student profiles matching the entered ID Number prior to lead submission.

#### Scenario: Existing Student Lookup by ID Number

- **WHEN** a user enters or updates the ID Number (`nationalId`) on the Create Lead form
- **THEN** the system SHALL perform a search for an existing student profile matching the ID Number only
- **AND** IF a match is found, prompt the user with a modal offering to link the lead to the existing student profile.
- **AND** IF the user cancels, revert the ID Number to its initial value.
- **AND** IF the user confirms, populate the lead form with the existing student's details.

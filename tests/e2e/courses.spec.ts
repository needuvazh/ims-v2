...
test.describe('Course Catalog UI Flow', () => {
  test('Branch Manager can list, create, and edit courses', async ({
    page,
  }) => {
    // 1. Log in
    ...
    await page
      .locator('div.cursor-pointer', { hasText: 'Information Technology' })
      .click();

    await page
      .locator('input[name="nameEnglish"]')
      .fill('Playwright Test Course');
    await page
      .locator('input[name="nameArabic"]')
      .fill('دورة اختبار بلاي رايت');
    await page
      .locator('textarea[name="descriptionEnglish"]')
      .fill('E2E automation course for testing.');
    await page
      .locator('textarea[name="descriptionArabic"]')
      .fill('دورة آلية للاختبارات الكاملة.');

    // Select category and classification
    await page
      .getByRole('button', { name: 'Select category (Optional)' })
      .click();
    await page
      .locator('div.cursor-pointer', { hasText: 'Technology & Engineering' })
      .click();

    await page.getByRole('button', { name: 'Regular (Open Batches)' }).click();
    await page
      .locator('div.cursor-pointer', { hasText: 'Regular (Open Batches)' })
      .click();
    ...
    await page
      ...
      .getByRole('button', { title: 'Edit Course' })
      .click();
    ...
    await page
      .locator('input[name="nameEnglish"]')
      .fill('Playwright Test Course (Updated)');
    ...
    await expect(
      page.getByText('Playwright Test Course (Updated)'),
    ).toBeVisible();
  });
});

...
- Admin portal has sign-in, dashboard, identity management, organization management (branches, departments, classrooms, and hierarchy tree), UI preview, and basic protected layout surfaces.
...
2. **Organization completion [COMPLETED]**
   Institute, branch, department, classroom lifecycle, active dating, user presence checking, cascade status updating, soft delete rules, hierarchy visualization, branch scoping, API/server action contracts, and tests are complete.
...
Next OpenSpec proposals should be:

1. `build-enrollment-aggregate-foundation`
...
2. `implement-lead-admission-handoff`
   Scope: Inquiry, lead, source/stage, counselor assignment, follow-ups, conversion handoff, reports, audit logs, and tests.

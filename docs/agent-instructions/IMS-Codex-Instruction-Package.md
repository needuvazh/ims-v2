# IMS Codex Instruction Package

## 1. Brief Analysis Summary

The uploaded IMS materials define a single-client Institute Management System for coaching centers, skill training institutes, corporate training, individual training, and walk-in course completion programs. The domain is workflow-heavy and strongly relational: inquiries convert into admissions, admissions create students, students receive one or more enrollments, and enrollments drive finance, attendance, completion, and certificates.

The most important architectural implication is that Enrollment is the central aggregate. Regular, Corporate, and Walk-In flows should share one enrollment lifecycle instead of creating separate models. Walk-In is a specialized application flow over Course, Enrollment, Finance, Completion, and Certificate. Corporate employees begin as Corporate Participants and should become Students only when individual lifecycle tracking, login access, certificate history, future enrollment, or individual payment behavior requires it.

The recommended architecture is a TypeScript modular monolith in a monorepo. Next.js route handlers should act only as thin delivery adapters. Domain behavior belongs in domain-oriented packages with application services, aggregate methods, repositories, validation, authorization, domain events, audit logging, and tests. PostgreSQL and Prisma fit the relational, transactional nature of the system. Dynamic RBAC, branch-scoped authorization, effective dating, audit logs, and clean context ownership are mandatory quality concerns.

The documents do not justify microservices, CQRS, or event sourcing for the initial implementation. Use direct typed package calls, internal domain events, a transactional outbox, and background workers first. Add external queues, service extraction, online payment gateway automation, payroll, and AI intelligence only when later phases or operational evidence justify them.

## 2. Production-Ready `AGENTS.md`
...
Place the generated `AGENTS.md` at the root of the IMS repository before asking Codex to implement features. Keep the DDD Context Map, ER Model, and Technology Stack Recommendation available in the repository or project knowledge so future agents can inspect the source material when needed.

For best results, ask Codex to work context by context. Start with monorepo setup, shared kernel, database package, Identity & Access, Organization, and the Enrollment aggregate. Then proceed through Lead, Course & Batch, Finance, Attendance, Completion, Certificate, Trainer, Document, Audit, and later Phase 2 contexts.

When requesting implementation, name the bounded context and workflow explicitly. Example: "Implement the Admission & Enrollment application service for confirming an enrollment, including branch-scoped authorization, domain events, audit logging, Prisma persistence, and tests."

...
* **U\* (Restricted Update):** Approved rates in `TrainerCompensationRate` and active authorizations in `TrainerCourseAuthorization` are immutable or temporal configuration data. In-place overwrites of core/financial fields (such as `amount`, `paymentBasis`, `trainerId`, `courseId`, or `batchId`) are strictly blocked; updates can only be executed by setting `effectiveEndDate` and `status` to Cancelled/Inactive on the active record and creating a new record (versioning).

...
```json
{
  ...
  "status": "DRAFT",
  "recordCounts": { "total": 24, "present": 0, "late": 0, "absent": 0, "excused": 0, "notMarked": 24 },
  "version": 1
}
```
...
export const SearchAttendanceSessionsQuerySchema = PaginationQuerySchema.extend({
  ...
  if (value.attendanceDateFrom && value.attendanceDateTo && value.attendanceDateFrom > value.attendanceDateTo) {
    ctx.addIssue({ code: 'custom', path: ['attendanceDateTo'], message: 'ERR_ATT_INVALID_DATE_RANGE' });
  }
});
```
...
```json
{
    ...
    "items": [
      {
        ...
        "status": "SUBMITTED",
        "recordCounts": { "total": 24, "present": 20, "late": 2, "absent": 1, "excused": 1, "notMarked": 0 },
        "version": 4
      }
    ],
    "page": 1,
  ...
  "meta": { "requestId": "req_01JZ6G5W8Z7K9T6G3Q4S2P1M0N", "branchId": "8d983798-5a2f-4780-b546-a27674a33d0d", "generatedAt": "2026-07-04T08:30:00.000Z" }
}
```
...
```ts
export const AttendanceRecordMarkSchema = z.object({
  ...
  if (value.status === 'LATE' && !value.arrivalTime) ctx.addIssue({ code: 'custom', path: ['arrivalTime'], message: 'ERR_ATT_LATE_TIME_REQUIRED' });
  if (value.status === 'EXCUSED' && !value.excuseReasonCode) ctx.addIssue({ code: 'custom', path: ['excuseReasonCode'], message: 'ERR_ATT_EXCUSE_REASON_REQUIRED' });
});
...
export const DetectAttendanceAlertsSchema = z.object({
  ...
  reason: z.string().trim().min(5).max(500).default('Manual low attendance detection run')
});

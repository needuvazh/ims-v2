...
- **Alternative Flows:**
  - _A1: Batch Capacity Reached:_ If batch is full, system checks for waitlist. If waitlist is available, a waitlist entry is created and the enrollment cannot proceed as confirmed. If no waitlist, transaction aborts.
- **Postconditions:**
...
- **Alternative Flows:**
  - _A1: Refund Processing:_ If the withdrawal date is within the refund eligibility window, the system triggers a request to Module 09 (Finance) to calculate refund amounts and issue a credit note draft.
- **Postconditions:**
